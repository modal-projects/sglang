...
def _adder(
    *,
    ...
    first gates read, bypassing __init__ (which needs a real allocator). The
    rem_*_tokens properties read the allocator and tree cache, so those are
    stubbed to report `remaining_total` FULL and `remaining_swa` SWA tokens."""
    adder = object.__new__(PrefillAdder)
    adder.token_to_kv_pool_allocator = SimpleNamespace(
        available_size=lambda: remaining_total,
        full_available_size=lambda: remaining_total,
        swa_available_size=lambda: remaining_swa,
    )
...
def test_budget_state_kv_exhausted():
    adder = _adder(remaining_total=0)
...
def test_budget_state_swa_exhausted_while_full_has_room():
    adder = _adder(remaining_total=100, remaining_swa=0, is_hybrid_swa=True)
    assert adder.budget_state() == AddReqResult.NO_TOKEN
    assert adder.stop_cause == AdmissionBlockCause.SWA_TOKENS


def test_budget_state_non_swa_model_ignores_swa_pool():
    adder = _adder(remaining_total=100, remaining_swa=0, is_hybrid_swa=False)
...
def test_budget_state_kv_wins_over_mamba_when_both_are_out():
    adder = _adder(remaining_total=0, rem_mamba_slots=0)
    assert adder.budget_state() == AddReqResult.NO_TOKEN
    assert adder.stop_cause == AdmissionBlockCause.KV_TOKENS
...
def test_dllm_block_cause_blames_kv_when_the_full_pool_is_empty():
    adder = _adder(remaining_total=0)
    assert adder._dllm_block_cause() == AdmissionBlockCause.KV_TOKENS
    adder = _adder(remaining_total=100)
    assert adder._dllm_block_cause() == AdmissionBlockCause.DLLM_BUDGET
