"""Rank-local host images for full-model weight updates.

The image stores the final byte layout of every model-owned CUDA weight
storage. Background staging produces a complete image; the paused update only
copies it into the existing CUDA storages. This preserves tensor aliases and
CUDA graph addresses and is independent of the checkpoint or delta format.
"""
...
class IrrecoverableWeightCommitError(RuntimeError):
    """A host-to-device commit failed after it may have changed live weights."""


@dataclass(frozen=True)
class RankWeightSegment:
...
def iter_derived_weight_tensors(
    ...
    # Quantization methods may also be modules. Their hook takes the owning
    # layer as an argument and is invoked through layer.quant_method below.
    get_derived = (
        None
        if isinstance(module, QuantizeMethodBase)
        else getattr(module, "get_derived_weight_tensors", None)
    )
...
def build_rank_weight_image_plan(
    ...
    *,
...
) -> tuple[list[RankWeightSegment], int]:
    ...
    offset = 0
    segments: list[RankWeightSegment] = []
...
class RankWeightImage:
    ...
    def __init__(self, model: torch.nn.Module):
        ...
        if len(devices) != 1:
            raise RuntimeError(
                f"a rank weight image requires one CUDA device: {devices}"
            )
        self.device = devices.pop()
        self._segments_by_device_storage = {
            (
        ...
        self.segments_by_name: dict[str, RankWeightSegment] = {}
        for name, tensor in iter_weight_tensors(model):
    ...
    def storage_image_bytes(self, tensor: torch.Tensor) -> torch.Tensor:
        """Return an isolated host view for one live weight storage.
        ...
        if segment is None:
            raise KeyError("tensor storage is not part of the rank weight image")
    ...
    def _iter_chunks(
        ...
        for offset in range(0, segment.nbytes, _COPY_CHUNK_BYTES):
            nbytes = min(_COPY_CHUNK_BYTES, segment.nbytes - offset)
            yield (
                self.image[
                    segment.image_offset + offset : segment.image_offset
                    + offset
                    + nbytes
                ],
    ...
    def register_host_memory(self) -> dict[str, int | float | str]:
        ...
        if self.registered:
            return self.stats("register_rank_weight_image", 0.0)
        started = time.perf_counter()
        ...
        return self.stats("register_rank_weight_image", self.registration_wall_s)

    def copy_device_segments_to_image(
        self,
        segments: Iterable[tuple[RankWeightSegment, torch.Tensor]],
    ) -> None:
    ...
    def validate_commit(self, target_version: int) -> None:
        ...
        if not self.staged:
            raise RuntimeError("no rank weight image is staged")
        if self.target_version != target_version:
            raise RuntimeError(
        ...
        if not self.registered:
            raise RuntimeError(
                "staged host image is not registered; registration must finish "
                "before the engine pauses"
            )
            ...
            with torch.cuda.stream(self._stream):
                for segment in self._commit_segments:
                    for image_chunk, device_chunk in self._iter_chunks(
                        ...
                        device_chunk.copy_(image_chunk, non_blocking=True)
            self._stream.synchronize()
    ...
    def close(self) -> None:
        if self.registered:
            try:
                ...
                logger.exception("Failed to unregister rank weight image")
            self.registered = False
        ...
        if image_buffer is not None:
            try:
                ...
                logger.warning("rank weight image still has exported buffer views")
