...
def _load_module():
    if "disk_checkpoint_under_test" in sys.modules:
        return sys.modules["disk_checkpoint_under_test"]
    path = (
        Path(__file__).resolve().parents[3]
        / "python/sglang/srt/weight_sync/disk_checkpoint.py"
    )
    spec = importlib.util.spec_from_file_location("disk_checkpoint_under_test", path)
    module = importlib.util.module_from_spec(spec)
    sys.modules["disk_checkpoint_under_test"] = module
    spec.loader.exec_module(module)
    return module


disk_checkpoint = _load_module()
