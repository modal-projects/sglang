...
class Envs:
    ...
    # Fuse the small-batch decode {MoE all-reduce -> mlp_sconv -> attn_norm}
    # chain into ONE kernel (jit_kernel/inkling_ar_fused.py). Requires
    # SGLANG_OPT_USE_INKLING_CUSTOM_AR; falls back to the unfused chain outside
    # the decode small-batch band. Default on.
    SGLANG_OPT_USE_INKLING_FUSED_AR_SCONV_NORM = EnvBool(True)
    # Fuse the target-verify attention prologue {k/v sconv + save_windows +
    # qk-norm + KV-cache store} into one kernel (jit_kernel/inkling_attn_prologue.py),
    # overlapping rel_logits_proj on the alt stream. Default on; the fused KV
    # store is bf16-only, so FP8/MXFP8 pools transparently keep the backend
    # store (conv/windows/qk-norm stay fused). Gated to fa4 + head_dim==128 +
    # target-verify; every other path is untouched.
    SGLANG_OPT_USE_INKLING_FUSED_ATTN_PROLOGUE = EnvBool(True)
    # Sink shared experts as a stock FusedMoE (InklingSharedFusedMoE) vs the legacy
    # bmm-based InklingBatchDenseMLP. Default on; set False to fall back to the old class.
    SGLANG_OPT_USE_INKLING_SHARED_FUSED_MOE = EnvBool(True)
    # Default reasoning_effort for Inkling models when the request doesn't set one.
    # Accepts a float string in [0.0, 1.0] or "" (empty = unset / no default).
    SGLANG_INKLING_DEFAULT_REASONING_EFFORT = EnvStr("")
    # ====================================================================
