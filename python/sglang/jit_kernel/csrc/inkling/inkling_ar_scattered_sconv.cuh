...
#pragma unroll
      ...
      *reinterpret_cast<uint4*>(resid + base + c) =
...
#pragma unroll
      for (int j = 0; j < static_cast<int>(kSsVecElems); ++j) {
        ob[j] = __float2bfloat16(
            __bfloat162float(sh[j]) * rms * __bfloat162float(gh[j]));
      }
      *reinterpret_cast<uint4*>(nout + base + c) =
...
__global__ __launch_bounds__(1024, 1) void inkling_ar_scattered_sconv_kernel(
    const __grid_constant__ ArScatteredSconvParams p) {
  ...
  if (use_smw) {
    ...
    for (uint32_t i = threadIdx.x; i < nw; i += blockDim.x) smem_w[i] = wg[i];
    ...
    const uint32_t ncv8 = ncv * kSsVecElems;  // smem tile row stride (elems)
    {
      ...
      for (uint32_t i = threadIdx.x; i < items; i += blockDim.x) {
        ...
        const __nv_bfloat16* addr =
            mc_in + static_cast<int64_t>(t) * p.H + p.rank * p.Hc + lc;
        uint4 v;
        asm volatile(
    ...
    // ---- Stage B: conv from scratch + broadcast ----
    {
      ...
      for (uint32_t i = threadIdx.x; i < items; i += blockDim.x) {
        ...
        const int64_t cache_base =
            safe_idx[s] * p.cache_stride_slot + p.cache_col0 + lc;
        const uint32_t cv8 = lc - c0 * kSsVecElems;  // smem col offset (elems)
        const uint4 xt = p.use_tile
            ? *reinterpret_cast<const uint4*>(smem_x + (t - h0) * ncv8 + cv8)
            : *reinterpret_cast<const uint4*>(
                  scratch + static_cast<int64_t>(t) * p.Hc + lc);
...
#pragma unroll
        for (int k = 0; k < W1; ++k) {
          ...
          if (pos >= bos) {
            taps[k] = p.use_tile
                ? *reinterpret_cast<const uint4*>(smem_x + (pos - h0) * ncv8 + cv8)
                : *reinterpret_cast<const uint4*>(scratch + pos * p.Hc + lc);
          } else {
            const int64_t prow = pos - bos + W1;  // prefix row in [0, W1)
            taps[k] = cm ? *reinterpret_cast<const uint4*>(
                               &cache[cache_base + prow * p.cache_stride_w])
                         : make_uint4(0, 0, 0, 0);
          }
        }
...
#pragma unroll
            for (int k = 0; k < W1; ++k) {
              const float tap =
                  __bfloat162float(reinterpret_cast<const __nv_bfloat16*>(&taps[k])[j]);
              acc += tap * wt(j, k);
            }
        ...
        asm volatile(
            "multimem.st.relaxed.sys.global.v4.bf16x2 [%0], {%1,%2,%3,%4};" ::"l"(addr),
...
#pragma unroll
        for (int w = 0; w < W1; ++w) {
          old_reg[w] =
              *reinterpret_cast<const uint4*>(&wcache[cb + w * p.cache_stride_w]);
        }
...
#pragma unroll
              for (int w = 0; w < W1; ++w) {
                *reinterpret_cast<uint4*>(&wcache[db + w * p.cache_stride_w]) =
                    nv_reg[w];
              }
            }
          } else {
...
template <typename DType, uint32_t kNumGPU, int W, bool USE_SILU, bool USE_RESIDUAL, bool kPerBlockBarrier>
__global__ __launch_bounds__(1024, 1) void inkling_ar_stream_sconv_kernel(
    const __grid_constant__ ArScatteredSconvParams p) {
  static_assert(std::is_same_v<DType, __nv_bfloat16>, "multimem path is bf16-only");
  constexpr int W1 = W - 1;
...
#pragma unroll
      for (int k = 0; k < W1; ++k) {
        ...
        if (pos >= bos) {
          ...
          tp[k] = cm ? *reinterpret_cast<const uint4*>(
                           &cache[cb + prow * p.cache_stride_w])
                     : make_uint4(0, 0, 0, 0);
        }
      }
...
#pragma unroll
          for (int k = 0; k < W1; ++k) {
            const float tap =
                __bfloat162float(reinterpret_cast<const __nv_bfloat16*>(&tp[k])[j]);
            acc += tap * wt(j, k);
          }
          if constexpr (USE_SILU) acc = __fdividef(acc, 1.0f + __expf(-acc));
          if constexpr (USE_RESIDUAL) acc += xj;
      ...
      asm volatile(
          "multimem.st.relaxed.sys.global.v4.bf16x2 [%0], {%1,%2,%3,%4};" ::"l"(ao),
          "r"(reinterpret_cast<const uint32_t*>(yb)[0]),
          "r"(reinterpret_cast<const uint32_t*>(yb)[1]),
          "r"(reinterpret_cast<const uint32_t*>(yb)[2]),
          "r"(reinterpret_cast<const uint32_t*>(yb)[3])
...
#pragma unroll
      for (int k = 0; k < W1 - 1; ++k) taps[k] = taps[k + 1];
      taps[W1 - 1] = v;
    }
  }
...
struct ArScatteredSconvKernel {
  static void run(
      tvm::ffi::TensorView in_buffer,    // this rank's [T, H] view of the input symm region
    ...
    RuntimeCheck(weight.stride(1) == 1 && weight.stride(0) == W,
                 "weight must be contiguous [Hc, W] (smem stage)");
    ...
    if (full_update) {
      ...
      RuntimeCheck(cache.size(1) == W - 1 && cache.size(2) == H,
                   "full-width cache must be [slots, W-1, H]");
      RuntimeCheck(cache_col0 % kSsVecElems == 0 && cache_col0 + Hc <= H,
                   "cache_col0 must be an aligned in-range column offset");
      RuntimeCheck(!need_scratch, "verify (need_scratch) unsupported full-width");
    } else {
      RuntimeCheck(cache.size(1) == W - 1 && cache.size(2) == Hc,
                   "cache must be [slots, W-1, Hc]");
    ...
    if (do_norm) {
      ...
      RuntimeCheck(norm_residual.numel() == static_cast<int64_t>(T) * H &&
                       norm_out.numel() == static_cast<int64_t>(T) * H,
                   "norm residual/out must be [T, H]");
      RuntimeCheck(out_local.numel() == static_cast<int64_t>(T) * H,
                   "out_local must be [T, H]");
    }
    ...
    const auto launch = [&](auto kernel) {
      ...
      const uint32_t cvec_chunks =
          max(1u, min(cvecs, max(cap / max(1u, token_chunks),
                                 host::div_ceil(cvecs * min(chunk_rows, T) * 2u,
                                                block_size * max(1u, token_chunks)))));
    ...
    if (use_stream) {
      ...
      const auto launch_stream = [&](auto kernel) {
        ...
        uint32_t L = stream_walk > 0
            ? static_cast<uint32_t>(stream_walk)
            : max(48u, host::div_ceil(T * cvecs, threads));
...
__global__ __launch_bounds__(1024, 1) void inkling_ar_col_decode_kernel(
    const __grid_constant__ ColDecodeParams p) {
  static_assert(std::is_same_v<DType, __nv_bfloat16>, "multimem path is bf16-only");
  constexpr int W1 = W - 1;
  ...
  for (int i = 0; i < VPT; ++i) {
    if (act[i]) {
      res_raw[i] = *reinterpret_cast<const uint4*>(
          static_cast<const __nv_bfloat16*>(p.residual_in) +
          static_cast<int64_t>(t) * p.H + c0[i]);
    }
  }
...
#pragma unroll
        for (int k = 0; k < W1; ++k) {
          const float tap =
              __bfloat162float(reinterpret_cast<const __nv_bfloat16*>(&hist[k])[j]);
          acc += tap * cm * wt(k);
        }
        if constexpr (USE_SILU) acc = __fdividef(acc, 1.0f + __expf(-acc));
        if constexpr (USE_RESIDUAL) acc += xj;
...
#pragma unroll
      ...
      if (p.track_mask != nullptr &&
          static_cast<const bool*>(p.track_mask)[t]) {
        const int64_t dst = static_cast<const int64_t*>(
            p.track_dst)[static_cast<int64_t>(t) * p.track_dst_stride];
...
#pragma unroll
  for (int i = 0; i < VPT; ++i) {
    if (!act[i]) continue;
    const uint4 ov4 = *reinterpret_cast<const uint4*>(
        outp + static_cast<int64_t>(t) * p.H + c0[i]);
...
#pragma unroll
  for (int off = 16; off > 0; off >>= 1) sumsq += __shfl_down_sync(~0u, sumsq, off);
...
#pragma unroll
    for (int off = 16; off > 0; off >>= 1) total += __shfl_down_sync(~0u, total, off);
...
struct ColDecodeKernel {
  template <int VPT>
  static void launch(const ColDecodeParams& params, uint32_t t_num, uint32_t vecs,
                     DLDevice dev) {
    using namespace host;
    const uint32_t block = min(1024u, host::div_ceil(host::div_ceil(vecs, VPT), 32u) * 32u);
    constexpr auto kernel =
        inkling_ar_col_decode_kernel<DType, kNumGPU, W, USE_SILU, USE_RESIDUAL, VPT>;
  ...
  run(tvm::ffi::TensorView in_buffer,      // [T, H] view of the input symm region
    ...
    RuntimeCheck(Hc * kNumGPU == H, "Hc * world must equal H");
    RuntimeCheck(T >= 1 && T <= inkling_ar::kMaxBarrierBlocks,
                 "T must fit one barrier slot per token");
    RuntimeCheck(weight.size(0) == Hc && weight.size(1) == W &&
                     weight.stride(1) == 1 && weight.stride(0) == W,
    ...
    RuntimeCheck(host::div_ceil(vecs, static_cast<uint32_t>(vpt)) >= Hc / kSsVecElems,
                 "block too small for the own-shard lanes");
    switch (vpt) {
      case 1: launch<1>(params, T, vecs, dev); break;
      case 2: launch<2>(params, T, vecs, dev); break;
      case 3: launch<3>(params, T, vecs, dev); break;
      case 4: launch<4>(params, T, vecs, dev); break;
      default: RuntimeCheck(false, "unsupported vecs_per_thread (1/2/3/4)");
    }
  }
};
...
template <typename DType, uint32_t kNumGPU, int W, bool USE_SILU, bool USE_RESIDUAL, bool kPerBlockBarrier>
__global__ __launch_bounds__(1024, 1) void inkling_ar_banded_sconv_kernel(
    const __grid_constant__ ArBandedSconvParams p) {
  static_assert(std::is_same_v<DType, __nv_bfloat16>, "multimem path is bf16-only");
  constexpr int W1 = W - 1;
  ...
  if (scattered) {
    ...
    for (uint32_t it = ptid; it < items; it += pstride) {
      ...
      const uint4 v = *reinterpret_cast<const uint4*>(
          &csh[sidx[b] * p.cache_stride_slot + w * p.cache_stride_w + lc]);
      __nv_bfloat16* addr =
          wst + (static_cast<int64_t>(b) * W1 + w) * p.H + p.rank * p.Hc + lc;
      asm volatile(
          "multimem.st.relaxed.sys.global.v4.bf16x2 [%0], {%1,%2,%3,%4};" ::"l"(addr),
          "r"(v.x), "r"(v.y), "r"(v.z), "r"(v.w)
          : "memory");
  ...
  // ---- Phase 1: contiguous streamed reduce of [halo_lo, t_hi) x H ----
  {
    ...
    for (uint32_t i = v0 + gtid; i < v1; i += stride) {
      ...
      *reinterpret_cast<uint4*>(
          scratch + (static_cast<int64_t>(i) - base * vrow) * kSsVecElems) = v;
    }
  }
    ...
    for (uint32_t i = v0 + gtid; i < v1; i += stride) {
      ...
      if (p.debug_phase == 2) {  // raw copy broadcast: no conv/taps/weights
        ...
        const uint4 xr = *reinterpret_cast<const uint4*>(
            scratch + (srow2 * vrow + i % vrow) * kSsVecElems);
        __nv_bfloat16* addr2 = mc_out + static_cast<int64_t>(t) * p.H + c;
        asm volatile(
            "multimem.st.relaxed.sys.global.v4.bf16x2 [%0], {%1,%2,%3,%4};" ::"l"(addr2),
      ...
      const uint4 xt = *reinterpret_cast<const uint4*>(
          scratch + (srow * vrow + i % vrow) * kSsVecElems);
...
#pragma unroll
      for (int k = 0; k < W1; ++k) {
        const int64_t pos = static_cast<int64_t>(t) - (W1 - k);
        if (pos >= bos) {
          taps[k] = *reinterpret_cast<const uint4*>(
              scratch + ((pos - base) * vrow + i % vrow) * kSsVecElems);
        } else {
          const int64_t prow = pos - bos + W1;
          if (!cm) {
            taps[k] = make_uint4(0, 0, 0, 0);
            ...
            taps[k] = *reinterpret_cast<const uint4*>(
                wstage + (static_cast<int64_t>(s) * W1 + prow) * p.H + c);
          } else {
            taps[k] = *reinterpret_cast<const uint4*>(
                &cache[cache_base + prow * p.cache_stride_w]);
          }
...
#pragma unroll
        for (int w = 0; w < W1; ++w) {
          old_reg[w] =
              *reinterpret_cast<const uint4*>(&cache[cb + w * p.cache_stride_w]);
        }
        const uint4 zero = make_uint4(0, 0, 0, 0);
#pragma unroll
        for (int w = 0; w < W1; ++w) {
...
struct ArBandedSconvKernel {
  static void run(
      tvm::ffi::TensorView in_buffer,   // [T, H] view of the input symm region
    ...
    if (scattered) {
      ...
      RuntimeCheck(Hc % kSsVecElems == 0, "Hc must be a multiple of 8");
      RuntimeCheck(mc_wstage != 0 && local_wstage != 0 && mc_wstage % 16 == 0,
                   "scattered mode needs the window staging region");
    } else {
    ...
    RuntimeCheck(scratch.size(0) >= tpr + W - 1 && scratch.size(1) == H,
                 "scratch must be [tpr + W-1, H]");
    RuntimeCheck(mc_in % 16 == 0 && mc_out % 16 == 0, "multicast ptrs must be 16B aligned");
    RuntimeCheck(flag_ptrs_dev != 0 && state_ptr != 0, "barrier resources are null");
    RuntimeCheck(rank >= 0 && rank < kNumGPU, "rank out of range");
    ...
    const auto launch = [&](auto kernel) {
      ...
      const uint32_t num_blocks =
          nb_override > 0 ? min(static_cast<uint32_t>(nb_override), cap) : min(want, cap);
      const auto stream = LaunchKernel::resolve_device(device);
...
template <typename DType, uint32_t kNumGPU, int W, bool USE_SILU, bool USE_RESIDUAL, int VPT>
__global__ __launch_bounds__(1024, 1) void inkling_ar_ssconv_norm_decode_kernel(
    const __grid_constant__ SsconvNormDecodeParams p) {
...
#pragma unroll
  for (int i = 0; i < VPT; ++i) {
    ...
    if (mine[i]) {
      const int64_t cache_base =
          static_cast<int64_t>(slot_id) * p.cache_stride_slot + (c0[i] - shard_lo);
#pragma unroll
      for (int w = 0; w < W1; ++w) {
        hist_raw[i][w] =
            *reinterpret_cast<const uint4*>(&cp[cache_base + w * p.cache_stride_w]);
      }
    }
#pragma unroll
    for (int j = 0; j < static_cast<int>(kSsVecElems); ++j) {
      ...
      if constexpr (W == 4) {
        if (p.conv_weight_stride_d == W) {
          *reinterpret_cast<uint2*>(wtaps[i][j]) =
              *reinterpret_cast<const uint2*>(wp + wrow);
          continue;
        }
      }
#pragma unroll
      for (int w = 0; w < W; ++w) wtaps[i][j][w] = wp[wrow + w];
    }
  }
  ...
  auto* slot = static_cast<__nv_bfloat16*>(p.mc_stage) +
               (static_cast<uint64_t>(p.rank) * p.T + t) * p.D;
  auto* wslot = static_cast<__nv_bfloat16*>(p.mc_wstage) +
                static_cast<uint64_t>(t) * W1 * p.D;
  uint4 res_raw[VPT];
#pragma unroll
  for (int i = 0; i < VPT; ++i) {
    if (!act[i]) continue;
...
#pragma unroll
    for (int j = 0; j < static_cast<int>(kSsVecElems); ++j) xf[j] = 0.0f;
#pragma unroll
    for (uint32_t rr = 0; rr < kNumGPU; ++rr) {
      const uint4 d = *reinterpret_cast<const uint4*>(
          stage + (static_cast<uint64_t>(rr) * p.T + t) * p.D + c0[i]);
...
#pragma unroll
    for (int j = 0; j < 4; ++j) xb2[j] = __floats2bfloat162_rn(xf[2 * j], xf[2 * j + 1]);

    // ---- 4. sconv over the FULL width from the staged windows ----
    uint4 taps[W1];
#pragma unroll
    for (int w = 0; w < W1; ++w) {
      taps[w] = *reinterpret_cast<const uint4*>(
          wstage + static_cast<uint64_t>(w) * p.D + c0[i]);
    }
    float y[kSsVecElems];
#pragma unroll
    for (int j = 0; j < static_cast<int>(kSsVecElems); ++j) {
...
#pragma unroll
      for (int w = 0; w < W1; ++w) {
        const float h =
            __bfloat162float(reinterpret_cast<const __nv_bfloat16*>(&taps[w])[j]);
...
#pragma unroll
      for (int w = 0; w < W1; ++w) {
        const uint4 nv = (w < W1 - 1) ? ((cm != 0.0f) ? hist_raw[i][w + 1] : zero)
                                      : *reinterpret_cast<const uint4*>(xb2);
...
#pragma unroll
    for (int j = 0; j < static_cast<int>(kSsVecElems); ++j) {
      ...
      r[i][j] =
          yb + __bfloat162float(reinterpret_cast<const __nv_bfloat16*>(&res_raw[i])[j]);
      sumsq += r[i][j] * r[i][j];
    }
  }
...
struct SsconvNormDecodeKernel {
  template <int VPT>
  static void launch(const SsconvNormDecodeParams& params, uint32_t t_num, uint32_t vecs,
                     DLDevice dev) {
    using namespace host;
    const uint32_t block = min(1024u, host::div_ceil(host::div_ceil(vecs, VPT), 32u) * 32u);
    constexpr auto kernel =
        inkling_ar_ssconv_norm_decode_kernel<DType, kNumGPU, W, USE_SILU, USE_RESIDUAL, VPT>;
    const auto stream = LaunchKernel::resolve_device(dev);
    LaunchKernel(dim3{t_num}, dim3{block}, stream)(kernel, params);
  }
  ...
  run(tvm::ffi::TensorView in,
    ...
    RuntimeCheck(hc % kSsVecElems == 0, "Hc must be a multiple of 8");
    RuntimeCheck(t_num >= 1 && t_num <= inkling_ar::kMaxBarrierBlocks,
                 "T must be in [1, kMaxBarrierBlocks]");
    RuntimeCheck(d_num % kSsVecElems == 0, "D must be a multiple of 8");
    RuntimeCheck(cache.size(1) == W - 1, "cache must be [pool, W-1, Hc]");
    RuntimeCheck(cache.stride(2) == 1, "cache must be channel-contiguous");
    RuntimeCheck(conv_weight.size(0) == d_num && conv_weight.size(1) == W,
                 "conv_weight must be FULL [D, W]");
    RuntimeCheck(norm_weight.numel() == d_num, "norm_weight must be [D]");
    RuntimeCheck(mc_stage_ptr % 16 == 0 && mc_wstage_ptr % 16 == 0,
                 "staging ptrs must be 16B aligned");
