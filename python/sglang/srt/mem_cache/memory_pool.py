...
class MambaPool:
    ...
    # Both properties are pool-stable (conv tensors don't move after allocation)
...
class MHATokenToKVPoolMXFP8(MHATokenToKVPool):
        ...
        with self.memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            with (
                ...
                if self.mxfp8_sf_interleaved:
                    ...
                    k_sf_shape = (num_pages, n, self.MXFP8_SCALE_BLOCK_SIZE, chunk, k_sf_dim)
                    v_sf_shape = (num_pages, n, self.MXFP8_SCALE_BLOCK_SIZE, chunk, v_sf_dim)
                else:
    ...
    def _write_scales(self, idx, loc, k_scale, v_scale):
        ...
        if self.mxfp8_sf_interleaved:
            from sglang.tml.kernels.mxfp8_interleave_sf import store_sf_interleaved
    ...
    def _read_sf_interleaved(self, sf_buf: torch.Tensor, loc: torch.Tensor):
        ...
        chunk = self.page_size // self.MXFP8_SCALE_BLOCK_SIZE
        ipos = ((off % self.MXFP8_SCALE_BLOCK_SIZE) * chunk + (off // self.MXFP8_SCALE_BLOCK_SIZE)).long()
