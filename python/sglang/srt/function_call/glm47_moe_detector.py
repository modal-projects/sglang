...
class Glm47MoeDetector(BaseFormatDetector):
            ...
            if self._stream_state in [StreamState.INIT, StreamState.BETWEEN]:
                ...
                if self._xml_tag_buffer.endswith("</arg_value>"):
                    ...
                    if value_type in ("auto", "number", "integer"):
                        # The complete value determines its JSON type. Once
                        # streamed, an unquoted prefix cannot be taken back.
    ...
    def _process_arguments_streaming(
        ...
        if self._buffer_arguments:
            # A discriminator may arrive after the value whose type it selects.
            return None
