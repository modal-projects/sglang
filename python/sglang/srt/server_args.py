...
class ServerArgs:
    ...
    def _handle_piecewise_cuda_graph(self):
        ...
        if self.lora_paths or self.enable_lora:
            self.disable_piecewise_cuda_graph = True
        # 8. GGUF quantized models (custom dequant ops unsupported by torch.compile)
        if (
            ...
            self.disable_piecewise_cuda_graph = True
        # 9. DLLM (diffusion LLM) models (context manager in forward breaks dynamo)
        if self.dllm_algorithm is not None:
            self.disable_piecewise_cuda_graph = True
        # 10. CPU offload (breaks dynamo)
        if self.cpu_offload_gb > 0 or self.enable_hierarchical_cache:
            self.disable_piecewise_cuda_graph = True
        # 11. Deterministic inference
        if self.enable_deterministic_inference:
            self.disable_piecewise_cuda_graph = True
        # 12. PD disaggregation
        if self.disaggregation_mode != "null":
            self.disable_piecewise_cuda_graph = True
        # 13. Symmetric memory (torch.cuda.use_mem_pool is untraceable by dynamo)
        if self.enable_symm_mem:
            self.disable_piecewise_cuda_graph = True
        # 14. Expert distribution recorder
