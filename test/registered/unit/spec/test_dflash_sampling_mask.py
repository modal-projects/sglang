...
class TestDFlashSamplingMask(CustomTestCase):
    def test_sampling_mask_stays_bounded_until_result_processing(self):
        pending = build_dflash_sampling_mask_output(
            target_probs=torch.tensor([[[0.25, 0.0, 0.75, 0.0]]]),
        ...
        self.assertEqual(pending.support_ids.shape[-1], 3)
        masks, logprobs = pending.finalize()
        self.assertEqual(set(masks[0][0]), {0, 2})
    ...
    def test_filtered_target_probabilities(self):
        ...
        self.assertEqual([set(mask) for mask in masks[0]], [{1, 3}, {0, 2, 4}])
    ...
    def test_one_oversized_token_rejects_its_whole_request(self):
        ...
        self.assertEqual(set(masks[1][0]), {0, 1})
