...
def _get_chunked_embedding_by_item(
    ...
    if sync_group is not None:
        ...
        # Repeated keys can include a hit followed by a local miss after a
        # row-count mismatch evicts the entry. Re-encode every occurrence.
        miss_items = [
            (idx, item, start, end)
            for idx, item, start, end in overlapping
        ...
        hit_entries = [
            (key, item) for key, item in hit_entries if key not in global_misses
        ]
