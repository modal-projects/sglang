...
class LoRAMemoryPool:
    ...
    def is_shared_moe_module(module_name: str) -> bool:
        """Whether this buffer belongs to a shared-expert FusedMoE (a second
        FusedMoE per layer, e.g. Inkling's sink; sharded over the full TP group)."""
    ...
    def _has_shared_fused_moe(base_model: torch.nn.Module) -> bool:
        """Whether the model serves its shared experts as a second FusedMoE per
        layer (duck-typed marker set by e.g. InklingSharedFusedMoE)."""
        return any(
    ...
    def get_lora_A_shape(
        ...
        c = get_stacked_multiply(module_name, base_model)
        # Routed MoE shards along moe_tp_size, not the outer tp_size; *_shared_moe
        # modules shard over the full TP group at EP=1 (see InklingSharedFusedMoE).
        effective_tp_size = (
            self.tp_size
        ...
        def init_buffer(
            ...
            for module_name in target_modules:
                ...
                if module_name in ambiguous_modules and has_moe:
                    ...
                    # Shared-expert FusedMoE version (4D, second FusedMoE per
                    # layer, e.g. Inkling's sink experts).
    ...
    def load_lora_weight_to_buffer(
        ...
        for layer_id in range(self.num_layer):
            ...
            for name, weights in layer_weights.items():
                ...
                if "shared_experts." in name and weights.dim() == 3:
                    # 3D shared-expert FusedMoE weight (e.g. Inkling's sink); must precede
                    # the generic "experts" checks or it collides into routed *_moe buffers.
                    target_module = target_module + "_shared_moe"
                    if "lora_A" in name:
            ...
            for module_name, module in cur_layer_modules.items():
                ...
                if isinstance(module, FusedMoEWithLoRA):
                    # Routed experts shard along moe_tp_size (= tp // ep // dp); a shared
                    # FusedMoE (is_shared_fused_moe) shards over full TP at EP=1.
                    is_shared = module.base_layer.is_shared_fused_moe
                    key_suffix = "_shared_moe" if is_shared else "_moe"
                    moe_tp_rank = module.base_layer.moe_tp_rank
