...
def helion_aot_autotune(
    ...
    def inner_autotune(kernel: helion.Kernel):
        ...
        def wrapped_kernel_key(*inps: Any) -> tuple[Any, Any, Any]:
            ...
            hash_key = tuple(i.itemsize if isinstance(i, torch.dtype) else i for i in hash_key)
        ...
        def get_configs_from_autotuning(autotune_mode: AOTAutotuneMode):
            ...
            inputs = sorted(list(primary_inputs()), key=lambda x: wrapped_kernel_key(*x)[0])
            ...
            for idx, (key, timing) in enumerate(input_timings):
                logger.info(f"Key {key} timings: {' '.join([f'{i:.5f}' for i in timing])}")
        ...
        def default_int64_threshold(*args: Any) -> bool:
            return any([i.numel() >= 2**31 for i in args if isinstance(i, torch.Tensor)])
        ...
        def wrapped_func(*args: Any):
            ...
            if key in cached_kernels:
                ...
                return out
            if has_int64:
                kernel.settings = dataclasses.replace(kernel.settings, index_dtype=torch.int64)
            ...
            if key not in cached_kernels and cur_kernel_key in hash_configs:
                ...
                def config_key_sort(config_key: tuple[Any, Any, Any]) -> tuple[Any, ...]:
                    return (
                ...
                if len(used_config) == 3:
                    assert used_config[2] == cur_kernel_key[2], "Exact key not found in configs"
