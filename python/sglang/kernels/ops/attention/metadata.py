...
def _draft_extend_metadata_kernel(
    ...
    # 2. Page-table gather for this batch row and column chunk
    if max_seq_pages == 0:
        return
...
def draft_extend_set_metadata(
    ...
    """Fused, graph-recordable DRAFT_EXTEND_V2 metadata update (one launch):
      ...
      4. page_table[:, :max_seq_pages] = req_to_token[pool_idx, ::page_size] // page_size

    Gathers the full static page-table width so replays stay correct for any
    seq len without a host max.
    """
    assert (
    ...
    if max_seq_pages == 0:
        ...
        grid = (batch_size, triton.cdiv(max_seq_pages, BLOCK_COLS))
