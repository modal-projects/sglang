...
from sglang.tml.layers.norm import RMSNorm
...
class InklingQKVRLinear(MergedColumnParallelLinear):
    ...
    (#15, via ``_kv_total_for_sizing``). This subclass carries the head geometry so
...
class InklingAttention(nn.Module):
    ...
    def _fused_attn_prologue_verify(self, q, k, v, forward_batch):
        ...
        k_cache, ci, cm, kw, k_inter = self.k_sconv.verify_fused_ar_inputs(forward_batch)
        ...
        if do_store and self.is_local:
            ...
            loc = get_attn_backend().forward_metadata.swa_out_cache_loc
        else:
            loc = forward_batch.out_cache_loc
        es = q.element_size()
        q, k, v = inkling_attn_prologue_verify(
            q, k_cache, v_cache, ci, cm, kw, vw, k_inter, v_inter,
            self._qk_gamma_bf16[0], self._qk_gamma_bf16[1],
    ...
    def forward(
        ...
        *,
        ...
        if fused_prologue:
            ...
            # {k/v sconv + save_windows + qk-norm (+ KV store)} on the current one.
