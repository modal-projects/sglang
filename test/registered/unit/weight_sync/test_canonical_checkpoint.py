...
def test_indexed_checkpoint_is_cached_once_with_zero_copy_views(tmp_path):
    ...
    try:
        for name, tensor in expected.items():
            ...
            assert getattr(cached_tensor, STABLE_WEIGHT_SOURCE_ATTR, False)
        stats = cached.stats()
...
def test_indexed_shard_may_link_to_huggingface_blob_store(tmp_path):
    checkpoint = tmp_path / "checkpoint"
    checkpoint.mkdir()
    outside = tmp_path / "outside.safetensors"
    save_file({"a": torch.arange(3)}, outside)
    (checkpoint / "linked.safetensors").symlink_to(outside)
    _write_index(checkpoint, {"a": "linked.safetensors"})
...
def test_update_must_finish_at_the_declared_target(tmp_path):
    shared_memory = tmp_path / "shared-memory"
    shared_memory.mkdir()
    save_file({"a": torch.arange(3)}, tmp_path / "model.safetensors")
