...
class Scheduler(
    ...
    def run_event_loop(self) -> None:
        ...
        # torch.cuda.Stream() draws round-robin from a fixed 32-slot per-device
        # pool, so schedule_stream (drawn here, after CUDA-graph capture) can alias
        # forward_stream (drawn in ModelRunner.__init__) when the number of pool
        # draws in between wraps the ring back to forward_stream's slot. Aliasing
        # makes the two the SAME underlying stream, collapsing the overlap
        # scheduler's schedule work onto forward_stream. Redraw until distinct.
        _redraws = 0
        while (
