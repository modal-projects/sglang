...
def test_preaborted_sibling_preserves_active_session_owner():
    """Rejecting a sibling must not clear the active turn's session ownership."""
    inner = _FakeInnerCache(None, _FakeAllocator(), page_size=1)
    tree_cache = StreamingSession(inner)
    slot = SessionSlot(kv=ReqKvInfo(req_pool_idx=0))
    tree_cache.slots["session-a"] = slot
    session = Session(16, "session-a", streaming=True)
    session._inflight = True
    session._inflight_rid = "active"
    rejected = _FakeReq("session-a", req_pool_idx=1, committed=1, allocated=1)
    rejected.session = session
    rejected.rid = "rejected"
    rejected.to_finish = FINISH_ABORT("session busy")

    assert tree_cache.find_active_slot(rejected) is None
    assert rejected.session is None
    assert session._inflight
    assert session._inflight_rid == "active"
    assert tree_cache.slots["session-a"] is slot


def test_preaborted_first_turn_detaches_without_cached_kv():
    """A rejected first turn must release ownership even before a KV slot exists."""
    for slot in (None, SessionSlot()):
        inner = _FakeInnerCache(None, _FakeAllocator(), page_size=1)
        tree_cache = StreamingSession(inner)
        if slot is not None:
            tree_cache.slots["session-a"] = slot
        session = Session(16, "session-a", streaming=True)
        session._inflight = True
        session._inflight_rid = "rejected"
        rejected = _FakeReq("session-a", req_pool_idx=1, committed=1, allocated=1)
        rejected.session = session
        rejected.rid = "rejected"
        rejected.to_finish = FINISH_ABORT("input too long")
...
def test_session_controller_plan_reap_defers_application():
    ...
    assert plan == io_struct.SessionReapPlan(
        deferred=["ready"],
        timed_out=["timed-out"],
    )
...
def test_deferred_reap_waits_for_active_turn_after_finished_history():
    """A saved finished turn does not make a session with a live append reapable."""
    controller, tree_cache = _controller_with_deferred_session()
    session = controller.sessions["deferred"]
    session._inflight = True
    session._inflight_rid = "active"

    controller.maybe_reap(10)
    assert "deferred" in controller.sessions
    assert tree_cache.released == []

    session.abort_req("rejected")
    controller.maybe_reap(12)
    assert "deferred" in controller.sessions
    assert tree_cache.released == []

    session.abort_req("active")
    controller.maybe_reap(14)
...
def test_session_controller_apply_reap_is_rank_symmetric():
    ...
    plan = io_struct.SessionReapPlan(deferred=["deferred"], timed_out=[])
...
def test_request_receiver_classifies_session_reap_plan_as_work():
    ...
    plan = io_struct.SessionReapPlan(deferred=[], timed_out=[])
...
def test_skipped_receive_cycle_still_reaps_deferred_close():
    ...
    assert recv_reqs == [io_struct.SessionReapPlan(deferred=["deferred"], timed_out=[])]
...
def test_skipped_receive_cycle_broadcasts_local_aborts():
    ...
    assert recv_reqs == [
        io_struct.SessionReapPlan(deferred=["deferred"], timed_out=[]),
        abort,
    ]


def test_receive_cycle_appends_reap_plan_to_pulled_reqs():
    """A normal receive cycle broadcasts the leader's reap plan with requests."""
    controller, _ = _controller_with_deferred_session()
    receiver = _make_single_rank_receiver(
