...
__device__ __forceinline__ float inkling_gemv_partial(
    const bf16_t* __restrict__ x_row, const bf16_t* __restrict__ w_row, int v_lo, int v_hi, int lane) {
  float acc = 0.0f;
...
void dispatch_inkling_gate_topk_renorm_v2(
  ...
  auto launch = [&](auto kernel) {
    LaunchKernel(grid, block, device).enable_pdl(enable_pdl)(
        kernel, logits, bias, global_scale, routed_w, indices, packed, shared_w, tokens, logits_stride_m, route_scale);
  };
...
void dispatch_inkling_gate_gemv(
  ...
  auto launch = [&](auto kernel) {
    LaunchKernel(grid, block, device).enable_pdl(enable_pdl)(
        kernel, x, weight, bias, global_scale, logits, routed_w, indices, packed, shared_w, ticket, tokens, route_scale);
  };
...
void inkling_gate_topk_renorm(
  ...
  TensorMatcher({M, kInklingTotalExperts}).with_strides({-1, 1}).with_dtype<fp32_t>().with_device<kDLCUDA>(device_).verify(logits);
