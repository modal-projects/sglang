...
class TestReloadLifecycle(CustomTestCase):
    ...
    def test_disk_reload_recaptures_decode_graph(self):
        ...
        runner.init_decode_cuda_graph = Mock()

        with patch.object(
            model_runner_mod,
            "get_model",
            return_value=SimpleNamespace(custom_weight_loader=[]),
        ):
            runner.init_weight_updater()

        runner.weight_updater.recapture_cuda_graph()
        runner.init_decode_cuda_graph.assert_called_once_with()
