...
def _scheduler_stub(cache):
    ...
    stub = SimpleNamespace(
        ...
        ps=SimpleNamespace(pp_size=1, tp_rank=0),
...
class TestSessionQueueAbort(CustomTestCase):
    ...
    def _assert_idle(self, observer, checker):
        ...
        pool = observer.req_to_token_pool
        for ids in (
            pool.free_slots,
            pool.mamba_allocator.free_slots.tolist(),
            observer.token_to_kv_pool_allocator.get_all_free_pages().tolist(),
        ):
            self.assertEqual(len(ids), len(set(ids)))
            self.assertTrue(all(idx > 0 for idx in ids))
    ...
    def test_transfer_queue_abort_in_decode_mode_finishes_session_turn(self):
        ...
        for deferred in (False, True):
            with self.subTest(deferred=deferred):
                ...
                manager.enable_deferred_decode_kv_release = deferred
                decode_req = SimpleNamespace(req=req, kv_receiver=Mock())
                decode_req.kv_receiver.kv_mgr = manager
                decode_req.kv_receiver.abort_notified = True
                decode_req.kv_receiver.bootstrap_infos = [{"rank": 0}, {"rank": 1}]
                scheduler = _scheduler_stub(cache)
                scheduler.disaggregation_mode = DisaggregationMode.DECODE
                scheduler.disagg_decode_prealloc_queue = SimpleNamespace(
                ...
                # and releases its KV.
                self.assertTrue(session.has_unfinished_request())
                self.assertIsNotNone(own_media.feature)
                self.assertEqual(
                    scheduler.disagg_decode_transfer_queue.queue, [decode_req]
                )
                send_output.assert_not_called()
                self.assertIsNotNone(req.kv.req_pool_idx)
                ...
                transfer_queue.enable_deferred_kv_release = deferred
                transfer_queue.deferred_kv_release_timeout = 60
                transfer_queue._deferred_releases = []
    ...
    def test_transfer_failure_of_statusless_internal_abort_counts_as_failure(self):
        ...
        self.assertFalse(req.user_aborted)
    ...
    def test_dllm_direct_abort_keeps_timeout_reason(self):
        ...
        self.assertEqual(sent.finished_reason, req.finished_reason.to_json())
    ...
    def test_dllm_deferred_abort_conserves_real_pools_for_every_result_shape(self):
        """Pending result ownership survives repeated cancel, empty output and replay."""
        from sglang.srt.dllm.mixin.scheduler import DllmManager, SchedulerDllmMixin

        for shape in ("global_empty", "row_empty", "zero_accept", "tokens"):
            with self.subTest(shape=shape):
                _, cache, allocator, pool, observer, checker, session = (
                    self._setup_first_turn()
                )
                req = session.create_req(
                    _recv("turn-2", [32, 33]), tokenizer=None, vocab_size=VOCAB_SIZE
                )
                req.init_next_round_input(cache)
                ...
                scheduler.result_queue = deque([(batch, None)])
                ...
                Scheduler.abort_request(
                    scheduler,
                    AbortReq(
                        rid=req.rid, abort_message="Request running timeout reached."
                    ),
                )
                reason = req.to_finish
                Scheduler.abort_request(scheduler, AbortReq(rid=req.rid))
                self.assertIs(req.to_finish, reason)
                self.assertTrue(session.has_unfinished_request())
                self.assertTrue(req.kv.holds_kv)
                tokens = (
                    []
                    if shape == "global_empty"
                    else [
                        torch.tensor(
                            [] if shape == "row_empty" else [40, 41, 42, 43],
                            dtype=torch.long,
                        )
                    ]
                )
    ...
    def test_unadmitted_abort_releases_real_session_resources(self):
        ...
        for route in ("sampling_mask", "prefill_capacity", "decode_capacity"):
            with self.subTest(route=route):
                ...
                scheduler = _scheduler_stub(cache)
                scheduler.output_streamer = SimpleNamespace(stream_output=Mock())
                if route == "sampling_mask":
                    scheduler._reject_sampling_mask_request(req, "Invalid request")
                else:
                    queue_type = (
                        PrefillBootstrapQueue
                        if route == "prefill_capacity"
                        else DecodePreallocQueue
                    )
                    queue = queue_type.__new__(queue_type)
                    queue.scheduler = scheduler
                    queue.max_total_num_tokens = 1
                    scheduler.enable_hisparse = False
    ...
    def test_rejected_admission_returns_only_request_owned_mamba(self):
        """Capacity rollback frees a new Mamba slot and retains a restored slot."""
        ...
        for restored in (False, True):
            with self.subTest(restored=restored):
                ...
                if not restored:
                    ...
                    session = Session(
                        capacity_of_str_len=0, session_id="fresh", streaming=True
                    )
                req = session.create_req(
                    _recv("turn-2", list(range(17))),
                    tokenizer=None,
                    vocab_size=VOCAB_SIZE,
                )
                scheduler = _scheduler_stub(cache)
                ...
                scheduler.get_num_allocatable_reqs = lambda *a, **kw: 8
                scheduler.policy = SimpleNamespace(calc_priority=lambda *a, **kw: None)
                ...
                def reject(candidate, **kwargs):
                    self.assertTrue(candidate.kv.holds_mamba)
                    if not restored:
                        self.assertIsNotNone(candidate.kv.mamba_cow_src_index)
                        self.assertIsNone(candidate.kv.req_pool_idx)
                    return AddReqResult.NO_TOKEN

                adder = SimpleNamespace(can_run_list=[], add_one_req=reject)
                ...
                with patch(
                    ...
                    result, _ = Scheduler._get_new_batch_prefill_raw(
                        scheduler, None, batch
                    )
                self.assertIsNone(result)
                self.assertIsNone(req.kv.mamba_cow_src_index)
                self.assertFalse(req.kv.mamba_needs_clear)
                self.assertEqual(pool.mamba_allocator.available_size(), before)
                self.assertEqual(req.kv.holds_mamba, restored)
                self.assertTrue(session.has_unfinished_request())
                Scheduler.abort_request(scheduler, AbortReq(rid=req.rid))
                self._assert_idle(observer, checker)

    def test_waiting_abort_releases_full_attention_session_row(self):
        """A restored attention-only turn must release its row without a Mamba marker."""
        ...
        for cache_type in (ChunkCache, RadixCache):
            with self.subTest(cache=cache_type.__name__):
                ...
                first = session.create_req(_recv("turn-1", [1, 2]), None, VOCAB_SIZE)
                _prefill(first, cache, allocator, pool)
                _finish_first_turn(first, cache)
                req = session.create_req(_recv("turn-2", [3]), None, VOCAB_SIZE)
                req.init_next_round_input(cache)
                self.assertTrue(req.kv.holds_kv)
                ...
                self.assertFalse(session.has_unfinished_request())
                self.assertFalse(req.kv.holds_kv)
                self.assertFalse(cache.has_slot(session.session_id))
