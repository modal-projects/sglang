...
__global__ __launch_bounds__(1024, 1) void inkling_ar_sconv_norm_kernel(
    const __grid_constant__ ArSconvNormParams p) {
...
#pragma unroll
  for (int i = 0; i < VPT; ++i) {
    ...
    const int64_t cache_base =
        static_cast<int64_t>(slot_id) * p.cache_stride_slot + c0[i];
#pragma unroll
    for (int w = 0; w < W1; ++w) {
      hist_raw[i][w] =
...
#pragma unroll
  for (int i = 0; i < VPT; ++i) {
    ...
    float xf[kVecElems];
#pragma unroll
    for (int j = 0; j < static_cast<int>(kVecElems); ++j) xf[j] = 0.0f;
#pragma unroll
...
#pragma unroll
      for (int w = 0; w < W1; ++w) {
        const float h =
            __bfloat162float(reinterpret_cast<const __nv_bfloat16*>(&hist_raw[i][w])[j]);
...
__global__ __launch_bounds__(1024, 1) void inkling_ar_sconv_norm_verify_kernel(
    const __grid_constant__ ArSconvNormVerifyParams p) {
  static_assert(std::is_same_v<DType, __nv_bfloat16>, "multimem push path is bf16-only");
  constexpr int W1 = W - 1;
  const uint32_t vecs = p.D / kVecElems;
...
#pragma unroll
      for (int w = 0; w < W; ++w) wtaps[j][w] = wp[wrow + w];
    }
  }
  ...
  if (active) {
    for (uint32_t t = blockIdx.x; t < p.T; t += stride_t) {
      ...
      asm volatile(
          "multimem.st.relaxed.sys.global.v4.bf16x2 [%0], {%1,%2,%3,%4};" ::"l"(slot),
          "r"(d.x), "r"(d.y), "r"(d.z), "r"(d.w)
          : "memory");
    }
  }
  ...
  inkling_ar::grid_system_barrier<kNumGPU>(p.state, p.flag_ptrs, p.rank, 0,
                                       /*publish_writes=*/true);
...
#pragma unroll
    for (uint32_t rr = 0; rr < kNumGPU; ++rr) {
      const uint4 d = *reinterpret_cast<const uint4*>(
          stage + (static_cast<uint64_t>(rr) * p.T + row) * p.D + c0);
      const auto* h2 = reinterpret_cast<const __nv_bfloat162*>(&d);
#pragma unroll
      for (int j = 0; j < 4; ++j) {
        const float2 f = __bfloat1622float2(h2[j]);
...
#pragma unroll
    for (int j = 0; j < 4; ++j) out2[j] = __floats2bfloat162_rn(xf[2 * j], xf[2 * j + 1]);
  };
...
#pragma unroll
        for (int iw = 0; iw < W1; ++iw) {
          ...
          if (shifted >= bos) {
            tap = __bfloat162float(
                reinterpret_cast<const __nv_bfloat16*>(xn2[W1 - 1 - iw])[j]);
          } else {
            const int prefix_pos = shifted - bos + W1;
            if (prefix_pos >= 0) {
              tap = cm * __bfloat162float(
                             reinterpret_cast<const __nv_bfloat16*>(&pref_raw[prefix_pos])[j]);
            }
          }
      ...
      if (valid) {
        auto* op = static_cast<__nv_bfloat16*>(p.inter_out) +
...
#pragma unroll
    ...
    float ss = sumsq;
#pragma unroll
    for (int off = 16; off > 0; off >>= 1) ss += __shfl_down_sync(~0u, ss, off);
...
struct ArSconvNormKernel {
  template <int VPT>
  static void launch(const ArSconvNormParams& params, uint32_t t_num, uint32_t vecs,
                     DLDevice dev, bool pdl) {
    using namespace host;
    const uint32_t block = min(1024u, div_ceil(div_ceil(vecs, VPT), 32u) * 32u);
    constexpr auto kernel =
        inkling_ar_sconv_norm_kernel<DType, kNumGPU, W, USE_SILU, USE_RESIDUAL, DO_TRACK, VPT>;
...
struct ArSconvNormVerifyKernel {
  ...
  run(tvm::ffi::TensorView in,
    ...
    const uint32_t d_num = static_cast<uint32_t>(D.unwrap());
    RuntimeCheck(q > 0 && t_num == b_num * static_cast<uint32_t>(q),
                 "T must equal B * draft_token_num");
    RuntimeCheck(t_num >= 1 && t_num <= inkling_ar::kMaxBarrierBlocks,
                 "T must be in [1, kMaxBarrierBlocks] (one barrier slot per token)");
    RuntimeCheck(d_num % kVecElems == 0, "D must be a multiple of 8");
    ...
    constexpr auto kernel =
        inkling_ar_sconv_norm_verify_kernel<DType, kNumGPU, W, USE_SILU, USE_RESIDUAL>;
    ...
    const uint32_t cap =
        host::runtime::get_sm_count(dev.unwrap().device_id) * max(1u, bps);
    const uint32_t grid = min(t_num, cap);
    LaunchKernel(dim3{grid}, dim3{block}, dev.unwrap())
        .enable_pdl(enable_pdl != 0)(kernel, params);
  }
};
