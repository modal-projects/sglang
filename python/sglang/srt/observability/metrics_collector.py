...
class RadixCacheMetricsCollector(_StatLoggerDIMixin):
    def __init__(
        ...
        self.kv_recomputed_idle_seconds = Histogram(
            name="sglang:kv_recomputed_idle_seconds",
            documentation="Seconds between a radix node's last access before "
            "its last copy was destroyed and an insert recreating the same "
            "prefix, observed once per dropped fragment that came back: the "
            "reuse gap the cache failed to span. Same clock and buckets as "
            'sglang:kv_age_seconds{event="hit"} (idle time at a hit), so '
            "the two together are the reuse-gap distribution of returning "
            "prefixes, uncensored by eviction. Always >= "
            "sglang:kv_reinsert_after_evict_seconds for the same fragment; "
            "reinserts more than SGLANG_KV_GHOST_LIST_TTL_S after the "
            "eviction are not seen.",
            labelnames=labels.keys(),
            buckets=list(KV_AGE_BUCKETS),
        )
        self.kv_recomputed_idle_tokens = Counter(
            name="sglang:kv_recomputed_idle_tokens_total",
            documentation="Token-weighted companion of "
            "sglang:kv_recomputed_idle_seconds: tokens recomputed after "
            "eviction, by the idle-time band they fell in (age_le is the "
            "band's upper edge in seconds, or +Inf; bands are not "
            "cumulative), and by the trigger that evicted them (as in "
            "sglang:kv_evicted_tokens_by_trigger_total). Add "
            'sglang:kv_age_tokens_total{event="hit"} for the tokens served '
            "at each idle time.",
            labelnames=list(labels.keys()) + ["age_le", "trigger"],
        )
        ...
        self._kv_recomputed_idle_hist = self.kv_recomputed_idle_seconds.labels(**labels)
        # (age_le, trigger) -> child, resolved on first use.
        self._kv_recomputed_idle_children = {}
    ...
    def observe_kv_reinsert_after_evict(
        ...
        self._kv_recomputed_idle_hist.observe(idle_seconds)
        key = (kv_age_bucket(idle_seconds), trigger)
        child = self._kv_recomputed_idle_children.get(key)
        if child is None:
            child = self._kv_recomputed_idle_children[key] = (
                self.kv_recomputed_idle_tokens.labels(
                    **self.labels, age_le=key[0], trigger=trigger
                )
            )
        child.inc(num_tokens)
