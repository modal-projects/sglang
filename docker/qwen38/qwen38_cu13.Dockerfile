...
#
# Base ships stock SGLang (editable at /sgl-workspace/sglang), stock DeepEP
# source (at /sgl-workspace/DeepEP), a released FlashInfer trio, the Rust
# toolchain at /root/.cargo, and the CUDA 13 toolchain (nvcc +
...
#   3. the FlashInfer nightly trio (python + cubin + jit-cache), replacing the
...
# ncclGinRequest_t, NCCL_GIN_*), which first shipped in NCCL 2.30 -- the
# base's 2.28 headers do not compile it. Install the pinned NCCL BEFORE the
# DeepEP build so the extension compiles and links against the same version
# that gets preloaded at run time.
...
# --- 2. NCCL preload path, staged where srt-slurm configs expect it ---
# (The 2.30.7 wheel itself is installed above, before the DeepEP build.)
...
# LD_PRELOAD of a dangling path fails silently at runtime. Both ends asserted.
# nvidia.nccl is a namespace package: __file__ is None, only __path__ is set.
RUN NCCL_LIB="$(python3 -c 'import os, nvidia.nccl; print(os.path.join(list(nvidia.nccl.__path__)[0], "lib"))')" && \
    test -d "${NCCL_LIB}" && \
...
# --- 3. FlashInfer nightly trio, over the base's release ---
