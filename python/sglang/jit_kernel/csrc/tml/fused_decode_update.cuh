// Inkling sconv: fused decode causal_conv1d + cache shift-update (+ optional track-copy)
// -- CUDA-JIT port of `_fused_causal_conv1d_update_decode_kernel` (fusion opt7/opt8).
//
...
#pragma unroll
  ...
  const __nv_bfloat162 xv =
      *reinterpret_cast<const __nv_bfloat162*>(&xp[static_cast<int64_t>(t) * p.x_stride_t + c0]);
...
#pragma unroll
  for (int iw = 0; iw < W1; ++iw) {
    const __nv_bfloat162 nv =
        (iw < W1 - 1) ? ((cm != 0.0f) ? hist[iw + 1] : zero) : xv;
