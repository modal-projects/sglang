...
def gemm_blockscaled(
    ...
    **kwargs,
    ...
    for kblock_idx in cutlass.range(num_kblocks, unroll_full=True):
        ...
        cute.gemm(tiled_mma, acc, tCrA[None, None, kblock_idx], tCrB[None, None, kblock_idx], acc)
...
def gemm_blockscaled_w_idx(
    ...
    """Blockscaled GEMM with optional A/B stage indexing."""
    rA = tCrA if const_expr(A_idx is None) else tCrA[None, None, None, A_idx]
    rB = tCrB if const_expr(B_idx is None) else tCrB[None, None, None, B_idx]
    gemm_blockscaled(tiled_mma, acc, rA, rB, tCtSFA, tCtSFB, zero_init=zero_init)
