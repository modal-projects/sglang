...
class PrefillAdder:
    def __init__(
        ...
        enable_admission_metrics: bool = False,
    ):
        self.enable_admission_metrics = enable_admission_metrics
        self.admission_stop_reason = None
    ...
    def _stop(self, result: AddReqResult, reason: str) -> AddReqResult:
        if self.enable_admission_metrics:
            self.admission_stop_reason = reason
        return result
    ...
    def _check_prefill_tile_budget(
        ...
        return self._stop(AddReqResult.OTHER, "prefill_tiles")
    ...
    def _swa_admission_gate(
        ...
        if not self._swa_req_never_fits(
            ...
            return self._stop(AddReqResult.NO_TOKEN, "swa_capacity"), chunk_tokens_limit
    ...
    def budget_state(self):
        ...
        if not no_token and self.rem_mamba_slots is not None:
            ...
            reason = "mamba_slots"
        if no_token:
            return self._stop(AddReqResult.NO_TOKEN, reason)

        if self.rem_input_tokens <= 0:
            return self._stop(AddReqResult.OTHER, "input_tokens")

        if self.dllm_config is not None:
            if self.rem_dllm_tokens <= 0:
                return self._stop(AddReqResult.OTHER, "dllm_tokens")
        else:
            if self.rem_chunk_tokens is not None and self.rem_chunk_tokens <= 0:
                return self._stop(AddReqResult.OTHER, "chunk_tokens")
    ...
    def add_dllm_staging_req(self, req: Req):
        ...
        if _rem_tokens <= 0:
            return self._stop(AddReqResult.NO_TOKEN, "dllm_tokens")
        ...
        return (
            self._stop(AddReqResult.NO_TOKEN, "dllm_tokens")
    ...
    def add_one_req_ignore_eos(self, req: Req):
        ...
        if paged_input > min(self.cur_rem_tokens, self.rem_total_tokens):
            return self._stop(AddReqResult.NO_TOKEN, "token_capacity")
        if self.is_hybrid_swa:
            if (
                self._swa_budget_for_req(
                ...
                return self._stop(AddReqResult.NO_TOKEN, "swa_capacity")
        ...
        if (self.prefill_delayer_single_pass is not None) and (
            ...
            return self._stop(AddReqResult.OTHER, "prefill_delay")
    ...
    def add_one_req(
        ...
        if (x := self.prefill_max_requests) is not None and len(self.can_run_list) >= x:
            return self._stop(AddReqResult.OTHER, "prefill_requests")
        ...
        with self._lock_node(req.last_node):
            ...
            if req.needs_host_load_back():
                ...
                if loaded is None:
                    return self._stop(AddReqResult.OTHER, "host_load")
