...
class PrefillCudaGraphRunner(BaseCudaGraphRunner):
    ...
    def capture_one_shape(self, size: int) -> None:
        ...
        if (
            isinstance(self.backend, BreakableCudaGraphBackend)
            and size == max(self.capture_num_tokens)
        ):
            # BCG capture pulls real workspace (MoE, attention transients)
            # into the shared graph pool — with captured attention AND with
            # eager-attn breaks (the warmup's cached blocks strand either
            # way; a DFLASH boot at ~4.7 GB avail watchdog-killed mid-capture
            # without this). Release the eager warmup's cached blocks first —
            # otherwise the global allocator strands memory the graph pool
            # can't use and capture OOMs/thrashes while plenty is
            # reclaimable. ONCE, before the first (largest) bucket: later
            # (smaller) buckets reuse cached blocks. Per-shape flushing
            # forces every bucket's warmup through synchronous cudaMalloc
            # against a nearly-full device — at mem-frac >=0.92 with dense
            # bucket lists that degraded to minutes per bucket.
