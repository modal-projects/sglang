...
from sglang.tml.kernels.sconv import (
    SconvDecodeMetadata,
    SconvExtendMetadata,
    causal_conv1d,
...
class ShortConvolution(nn.Module):
    ...
    def _prepare_cache_indices(
        ...
        mirroring PR #29867's once-per-step index resolution.
    ...
    def decode_fused_ar_inputs(self, forward_batch: ForwardBatch):
        """The tensors the fused decode {AR -> sconv -> add+norm} kernel needs
        (tml/kernels/comm.py::moe_ar_sconv_norm) -- exactly what the fused
        decode branch of ``forward`` below would pass to
        ``fused_causal_conv1d_update_decode``, including the shared per-step
        metadata cache behavior. Returns
    ...
    def verify_fused_ar_inputs(self, forward_batch: ForwardBatch):
        """Tensors for the fused target-verify {AR -> causal_conv1d ->
        save_intermediate_conv_windows -> add+norm} kernel
        (tml/kernels/comm.py::ar_sconv_norm_fused) -- mirrors the
        target_verify branch of ``forward`` below. Returns ``(sconv_cache,
