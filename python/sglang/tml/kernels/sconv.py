...
def _causal_conv1d_fwd_with_prefix_kernel(
    ...
    if not IS_DECODE:
        m_val = tl.load(cache_mask + si * stride_cm, mask=t_mask, other=0).to(x.dtype.element_ty)
    ...
    for iw in tl.static_range(W):
        ...
        w_val = tl.load(weight_base + iw * stride_weight_w, mask=d_mask, other=0).to(tl.float32)
        acc += tap * w_val[None, :]
...
def causal_conv1d(
    ...
    if (
        ...
        return _cuda_causal_conv1d(
            x, weight, sconv_cache, cache_mask, safe_idx, cu, si,
            activation=activation, use_residual=use_residual, is_decode=is_decode,
        )
    ...
    grid = lambda meta: (triton.cdiv(T, meta["BLOCK_T"]), triton.cdiv(D, meta["BLOCK_D"]))
...
def _update_sconv_cache_kernel(
    ...
    for w in tl.static_range(W_MINUS_1):
        ...
        for src_w in tl.static_range(W_MINUS_1):
            ...
            src_val = tl.load(cache_base + src_w * stride_cache_w, mask=bd_mask, other=0)
...
def update_sconv_cache(
    ...
    if (
        ...
        _cuda_update_sconv_cache(
            x, sconv_cache, cache_indices.to(torch.int32),
            has_initial_state, query_start_loc.to(torch.int32),
        )
        return
...
def fused_causal_conv1d_update_decode(
    ...
    if (
        ...
        return _cuda_fused_decode(
            x, weight, sconv_cache, cache_indices.to(torch.int32), cache_mask,
            activation=activation, use_residual=use_residual,
            track_mask=track_mask, track_indices=_ti,
        )
