...
if is_hip():
    ...
    # Wrapper with the deterministic variants. The kernel correctness tests
    # keep the direct sgl_kernel import above; the wrapper's default policy
    # reads published runtime context, which these kernel tests do not set
    # up, so the deterministic tests below always pass deterministic=
    # explicitly or pin SGLANG_RENORM_DETERMINISTIC (which short-circuits).
    from sglang.srt.layers.sampling_renorm import (
        top_k_renorm_prob as _wrap_top_k_renorm_prob,
    )
    from sglang.srt.layers.sampling_renorm import (
        top_p_renorm_prob as _wrap_top_p_renorm_prob,
    )
...
def test_top_p_renorm_probs_is_deterministic(batch_size, vocab_size):
    ...
    ref = _wrap_top_p_renorm_prob(probs, top_p, deterministic=True)
    for _ in range(30):
        out = _wrap_top_p_renorm_prob(probs, top_p, deterministic=True)
    ...
    with envs.SGLANG_RENORM_DETERMINISTIC.override(True):
        assert torch.equal(_wrap_top_p_renorm_prob(probs, top_p), ref)
...
def test_top_k_renorm_probs_is_deterministic(batch_size, vocab_size, k):
    ...
    ref = _wrap_top_k_renorm_prob(probs, top_k, deterministic=True)
    for _ in range(30):
        out = _wrap_top_k_renorm_prob(probs, top_k, deterministic=True)
        assert torch.equal(out, ref), "top_k_renorm_prob output changed between calls"
    with envs.SGLANG_RENORM_DETERMINISTIC.override(True):
        assert torch.equal(_wrap_top_k_renorm_prob(probs, top_k), ref)
    # default path agrees with the deterministic one up to rounding
    fast = _wrap_top_k_renorm_prob(probs, top_k, deterministic=False)
