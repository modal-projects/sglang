...
class InklingDetector(BaseReasoningFormatDetector):
    ...
    # Inkling output is not a single `<think>...</think>answer` span. It is a
    # sequence of typed content blocks, for example:
    #   <|content_thinking|>reasoning<|end_message|>
    #   <|content_text|>visible answer<|end_message|>
    #   <|content_model_end_sampling|>
    # The tokenizer must decode these special token IDs as their literal
    # strings so this text parser can route thinking blocks to reasoning_content
    # and text blocks to the visible assistant content.
    def __init__(
        self,
        stream_reasoning: bool = True,
        force_reasoning: bool = False,
    ...
    def parse_streaming_increment(self, new_text: str) -> StreamingParseResult:
        return self._parse_blocks(new_text)
