...
class KDAPrefillGraphMetadata:
    def __init__(self, max_tokens, max_requests, device):
        ...
        self.chunk_indices = torch.empty(
            (chunk_capacity(max_tokens, max_requests, KDA_CHUNK_SIZE), 2),
            dtype=torch.int32,
            device=device,
        )
        self.conv_indices = torch.empty(
            (chunk_capacity(max_tokens, max_requests, CAUSAL_CONV1D_CHUNK_SIZE), 2),
            dtype=torch.int32,
            device=device,
        )
    ...
    def update(self, lengths, cache_indices, prefix_lens, metadata):
        ...
        for size, target in (
            (KDA_CHUNK_SIZE, self.chunk_indices),
            (CAUSAL_CONV1D_CHUNK_SIZE, self.conv_indices),
        ):
...
def kda_prefill_graph_forward(layer, mixed_qkv, a, b, conv_pool, ssm_states, metadata):
    ...
    conv_indices = metadata.conv_indices[
        : chunk_capacity(tokens, requests, CAUSAL_CONV1D_CHUNK_SIZE)
    ]
    chunk_indices = metadata.chunk_indices[
        : chunk_capacity(tokens, requests, KDA_CHUNK_SIZE)
    ]
