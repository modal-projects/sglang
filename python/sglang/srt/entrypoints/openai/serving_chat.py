...
class OpenAIServingChat(OpenAIServingBase):
            ...
            async for content in self.tokenizer_manager.generate_request(
                ...
                if finish_reason_type:
                    ...
                    if finish_reason_type == "abort" and isinstance(
                        ...
                        yield f"data: {error}\n\n"
                        # Drop any buffered token ids so no sglext.input_ids /
                        # sglext.output_ids event is emitted after the error
                        # chunk. Finalization itself still runs, preserving the
                        # pre-existing usage/[DONE] framing on this error path.
                        input_ids = None
                        output_ids.clear()
                        break
    ...
    def _build_chat_response(
        ...
        if self._should_return_input_ids(request):
            # Aborted requests are finalized by
            # tokenizer_manager._handle_abort_req, which omits prompt_token_ids.
            prompt_token_ids = ret[0].get("prompt_token_ids")
