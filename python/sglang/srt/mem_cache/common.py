...
def _release_overallocated_kv_indices(
    ...
    # strip_thinking_cache and an overlap-scheduled decode that aborts before
    # appending its sampled token intentionally report a shorter cacheable
    # prefix. In the ordinary non-speculative path, physical allocation must
    # still exactly match the physical committed length.
    if spec_algo is None and not get_serving().strip_thinking_cache:
        assert (
            req.kv.kv_committed_len == end_p
        ), f"Unexpected overallocated KV cache, {req.kv.kv_committed_len=}, {req.kv.kv_allocated_len=}, {req.seqlen=}"
