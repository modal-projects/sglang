...
from sglang.tml.kernels.helion_utils import get_model_depths, helion_aot_autotune
...
@helion_aot_autotune(
    ...
    secondary_inputs=partial(
        silu_and_mul_inputs,
        sizes=[512] + [i * 96 for i in get_model_depths()] + list(range(1024, 8192 + 1, 1024)),
    ),
)
...
def _silu_and_mul_helion_interleaved_kernel(
    ...
    for batch_tile, hidden_tile in hl.tile([batch_size, half_hidden_size]):
        ...
        up_output = gateup_output[batch_tile, 2 * hidden_tile.index + 1].to(torch.float32)
...
def silu_and_mul_helion(
    ...
    """
    if use_interleaved:
        return _silu_and_mul_helion_interleaved_kernel(gateup_output, topk_weights, out_dtype)
    else:
        return _silu_and_mul_helion_non_interleaved_kernel(gateup_output, topk_weights, out_dtype)
...
def silu_and_mul_triton(
    ...
    if topk_weights is not None:
        assert topk_weights.is_contiguous(), f"{topk_weights.shape=} {topk_weights.stride()=}"
...
def _memset_block_metadata_kernel(
    ...
    if pid == 0:
        ...
        for off in range(0, E, BLOCK_SIZE_MEMSET):
            ...
            block_offs = (
                tl.cumsum(num_blocks_per_expert, 0)
...
def compute_expert_block_metadata(
    ...
    *,
    ...
    max_num_blocks = _get_max_num_blocks(
        num_routed_tokens, [block_size_m], num_experts
    )
    ...
    int64_index = (
        expert_block_offs.nbytes >= 2**31
        or expert_block_schedule.nbytes >= 2**31
    )
...
def compute_grouped_gemm_metadata(sorted_topk_ids: torch.Tensor, num_experts: int):
    ...
    expert_block_offs, expert_block_schedule = compute_expert_block_metadata(
        num_tokens_per_expert, num_routed_tokens
    )
    return (
...
def post_reorder(down_output: torch.Tensor, src2dst: torch.Tensor, topk_weights: torch.Tensor):
...
def _compute_expert_attrs(
    ...
    block_end = tl.minimum(block_start + BLOCK_SIZE_M, token_start_m + expert_num_tokens)
...
def grouped_gemm_triton(
    ...
    with torch.profiler.record_function(f"grouped_gemm_[M:{M},K:{K},E:{num_experts},N:{N}]"):
