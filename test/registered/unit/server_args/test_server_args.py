...
class TestDeepEPv2Args(CustomTestCase):
    ...
    # --- prefill capacity pre-check (per-sender chunk vs dispatch buffer cap) ---
    ...
    def test_prefill_dp4_tp4_scatter_reduces_sender_budget(self):
        # _handle_data_parallelism has already converted the global 8192-token
        # chunk to 2048 per DP worker. The SCATTERED MoE path then reduce-scatters
        # it over attention TP4, so each DeepEP sender sees only 512 tokens.
        args = self._args(
        ...
        with patch.dict(os.environ, {self._CAP_ENV: "512"}):
            args._handle_a2a_moe()
        self.assertEqual(args.moe_runner_backend, "deep_gemm")

    def test_prefill_dp4_tp4_scatter_uses_ceiling_sender_budget(self):
        args = self._args(
            moe_runner_backend="deep_gemm",
            chunked_prefill_size=2049,
            tp_size=16,
            dp_size=4,
            attn_cp_size=1,
            enable_dp_attention=True,
        ...
        with patch.dict(os.environ, {self._CAP_ENV: "512"}):
            with self.assertRaisesRegex(ValueError, "per-sender prefill"):
                args._handle_a2a_moe()
    ...
    def test_prefill_chunk_rejected_under_default_cap(self):
        ...
        args = self._args(moe_runner_backend="deep_gemm", chunked_prefill_size=1024)
        with self.assertRaisesRegex(ValueError, "prefill dispatch budget"):
            args._handle_a2a_moe()
