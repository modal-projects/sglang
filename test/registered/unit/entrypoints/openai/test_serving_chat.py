...
class ServingChatTestCase(unittest.TestCase):
    ...
    # ------------- sglext input/output ids tests -------------
    def _sglext_chunks(self, chunks):
        parsed = []
        for chunk in chunks:
            if not chunk.startswith("data: ") or chunk.strip() == "data: [DONE]":
                continue
    ...
    def _ids_ret(self, *output_ids_by_choice):
        return [
            {
                "text": "Answer",
    ...
    def test_non_streaming_ids_emit_sglext(self):
        ...
        response = self.chat._build_chat_response(
            req, self._ids_ret([5, 6, 7], [8, 9]), 123
        )
    ...
    def test_non_streaming_ids_not_returned_by_default(self):
        ...
        response = self.chat._build_chat_response(req, self._ids_ret([5, 6, 7]), 123)
    ...
    def test_ids_header_enables_flag(self):
        ...
        with patch.object(
            ...
            adapted_request, processed_request = self.chat._convert_to_internal_request(
                req, self.fastapi_request
            )
    ...
    def _run_ids_stream(self, chunk_output_ids, incremental, abort_output_ids=None):
        """Stream chunks whose output_ids follow incremental (delta) or
        non-incremental (full accumulated list) semantics, optionally followed by
        a graceful-abort chunk (finish_reason type 'abort', no status_code) whose
        output_ids is the producer's collapsed [last_token]. Returns the parsed
        sglext chunks."""
        self.tm.server_args.incremental_streaming_output = incremental

        async def _mock_generate():
            for i, ids in enumerate(chunk_output_ids):
                finished = i == len(chunk_output_ids) - 1 and abort_output_ids is None
                yield {
                    "text": "chunk",
                    "prompt_token_ids": [1, 2, 3],
                    "output_ids": list(ids),
            ...
            if abort_output_ids is not None:
                # Graceful abort terminal chunk (no status_code): falls through to
                # the normal finalization path, so its output_ids would corrupt
                # the accumulator if not skipped.
                yield {
                    "text": "chunk",
                    "prompt_token_ids": [1, 2, 3],
        ...
        loop = get_or_create_event_loop()
        return self._sglext_chunks(loop.run_until_complete(run_stream()))

    def test_streaming_output_ids_incremental_accumulates_deltas(self):
        sglext_chunks = self._run_ids_stream([[5, 6], [7]], incremental=True)
    ...
    def test_streaming_output_ids_non_incremental_keeps_latest_full_list(self):
        sglext_chunks = self._run_ids_stream([[5, 6], [5, 6, 7]], incremental=False)
    ...
    def test_streaming_output_ids_incremental_ignores_abort_chunk(self):
        ...
        sglext_chunks = self._run_ids_stream(
            [[5, 6], [7]], incremental=True, abort_output_ids=[7]
        )
    ...
    def test_streaming_output_ids_non_incremental_ignores_abort_chunk(self):
        ...
        sglext_chunks = self._run_ids_stream(
            [[5, 6], [5, 6, 7]], incremental=False, abort_output_ids=[7]
        )
    ...
    def test_streaming_abort_with_ids_enabled(self):
        """A terminal abort with ids enabled yields only an error and [DONE]."""
        err_msg = "Aborted by scheduler"
        err_code = HTTPStatus.INTERNAL_SERVER_ERROR
