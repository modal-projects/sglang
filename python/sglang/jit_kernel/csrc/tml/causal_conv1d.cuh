// Inkling sconv: depthwise causal conv1d (extend/prefill) with the W-1 prefix taps
// gathered directly from sconv_cache. CUDA-JIT port of the Triton kernel
// `_causal_conv1d_fwd_with_prefix_kernel` in python/sglang/tml/kernels/sconv.py.
//
// Semantics (matches benchmark/tml/fusion/opt1_causal_conv1d.py's fp32 reference).
...
//
// PERF: this conv is memory-light (~7% DRAM) -- the trap is being INSTRUCTION-bound.
// The control logic (shifted / bos compare / prefix decision) is channel-INDEPENDENT,
// so each thread processes 2 channels packed as bf16x2: the control is computed ONCE
// and applied to both lanes, halving issued instructions and vectorising the x/y
// loads/stores to 32-bit. Address math is int32 on the hot path (int64 only for the
// rare prefix gather); per-token metadata is hoisted to shared. Each thread owns its
// 2 channels across a strip of BLOCK_T tokens, loading its (BLOCK_T+W-1) x window into
// registers once and reusing it across the W taps.
  ...
  if (threadIdx.x < static_cast<uint32_t>(BT)) {
    ...
    if (t < T) {
      ...
      if constexpr (!IS_DECODE) {
        s_m[j] = static_cast<const bool*>(p.cache_mask)[static_cast<int64_t>(seq) * p.cache_mask_stride]
                     ? 1.0f
                     : 0.0f;
      }
    }
...
#pragma unroll
  for (int iw = 0; iw < W; ++iw) {
    wv[iw] = make_float2(__bfloat162float(wp[c0 * swd + iw]),
                         __bfloat162float(wp[(c0 + 1) * swd + iw]));
  }
      ...
      if (iw == W - 1) {
        ...
        if (shifted < bos && prefix_pos >= 0 && prefix_pos < (W - 1)) {  // rare: seq start
          const int64_t coff = static_cast<int64_t>(s_slot[j]) * p.cache_stride_slot +
                               static_cast<int64_t>(prefix_pos) * p.cache_stride_w +
