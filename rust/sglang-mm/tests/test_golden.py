...
def test_patchify_rgb_bit_exact(path):
    g = np.load(path)
    got = sglang.srt.multimodal._core.inkling.patchify_rgb(g["arr"], int(g["patch_size"]))
...
def test_decode_patchify_png_bit_exact(path):
    ...
    h, w, got = sglang.srt.multimodal._core.inkling.decode_patchify(g["png"].tobytes(), int(g["patch_size"]))
    assert (h, w) == (h_ref, w_ref)
    np.testing.assert_array_equal(got, g["bits"].reshape(-1))

def test_batch_matches_single():
    gs = [np.load(p) for p in GOLDENS]
    datas = [g["png"].tobytes() for g in gs]
    ps = int(gs[0]["patch_size"])
    for (h, w, bits), g in zip(sglang.srt.multimodal._core.inkling.decode_patchify_batch(datas, ps), gs):
