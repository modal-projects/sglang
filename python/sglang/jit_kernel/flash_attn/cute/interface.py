...
from sglang.jit_kernel.flash_attn.cute import fa_logging
...
from sglang.jit_kernel.flash_attn.cute.flash_fwd_sm100 import FlashAttentionForwardSm100, DescaleTensors
...
from sglang.jit_kernel.flash_attn.cute.flash_fwd_combine import FlashAttentionForwardCombine
from sglang.jit_kernel.flash_attn.cute.flash_fwd_mla_sm100 import FlashAttentionMLAForwardSm100

# SM100 head_dim=256 2CTA kernel imports
from sglang.jit_kernel.flash_attn.cute.sm100_hd256_2cta_fmha_forward import BlackwellFusedMultiHeadAttentionForward
...
def _flash_attn_fwd(
    ...
    if qk_blockscaled:
        ...
        assert q is not None and q.dtype in [torch.float8_e4m3fn], "qk_blockscaled Q must be float8_e4m3fn"
        ...
        if not v_blockscaled:
            assert v.dtype in [torch.float16, torch.bfloat16], "qk_blockscaled V must be float16 or bfloat16"
    else:
            ...
            if cached_prep is not None:
                ...
                if not is_fake_mode():
                    _flash_attn_fwd.compile_cache_prepare_shear_bias[compile_key_prepare](
                        cu_total_m_blocks_bias, cu_seqlens_q, blocks_to_batch_idx,
                    )
                if rel_bias_prep_cache is not None:
