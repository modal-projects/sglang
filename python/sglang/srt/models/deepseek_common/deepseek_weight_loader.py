...
def _index_expert_params_mapping(
    mappings: List[Tuple[str, str, int, str]],
) -> Dict[int, List[Tuple[str, str, int, str]]]:
    """Index expert mappings so loading is O(tensors), not O(tensors*experts)."""

    result = defaultdict(list)
    for mapping in mappings:
        result[mapping[2]].append(mapping)
    return dict(result)
...
def _expert_mapping_candidates(
    ...
    """Return the constant-sized mapping bucket for a checkpoint tensor.

    Keep the complete list as a compatibility fallback for non-standard expert
    names.  Standard ``experts.<integer>.`` checkpoints avoid scanning mappings
    for every other expert, which is particularly important for 100B+ MoE
    checkpoints containing hundreds of thousands of expert tensors.
    """
...
class DeepseekV2WeightLoaderMixin:
    ...
    def do_load_weights(
        ...
        expert_params_mapping_by_expert = _index_expert_params_mapping(
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            def submit_weight_load(
                *,
                ...
                batch_method = getattr(owner, "weight_loader_batch", None)
            ...
            if batched_weight_loads:
                for owner, calls in batched_weight_loads.items():
                    owner.weight_loader_batch(calls, executor=executor)
