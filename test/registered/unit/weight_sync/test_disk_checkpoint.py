...
class Publisher:
    """Build a base checkpoint and a published XOR delta lineage.

    Each version directory contains zstd-compressed per-tensor XOR differences
    and checksums of the resulting state in the safetensors metadata.
    """
...
class MaterializeTest(unittest.TestCase):
    ...
    def test_lineage_is_validated_before_mutating_the_checkpoint(self):
        self.materialize(0)
        ...
        with self.assertRaisesRegex(ValueError, "builds on v9, expected v1"):
            self.materialize(2)
        self.assertEqual(read_local(self.local), initial)
        self.assertEqual(disk_checkpoint._read_applied_version(self.local), 0)

    def test_folded_lineage_checks_the_final_target(self):
        rng = np.random.default_rng(37)
        self.pub.publish_delta(
            3, {"layer.a": rng.integers(0, 256, 4096, dtype=np.uint8).tobytes()}
        )
        self.pub.publish_delta(
            4, {"layer.a": rng.integers(0, 256, 4096, dtype=np.uint8).tobytes()}
        )
        shard = os.path.join(
            self.pub.source_dir,
            "weight_v000003",
        ...
        write_safetensors(
            shard,
            {"layer.a": zstandard.ZstdCompressor().compress(corrupted)},
    ...
    def test_invalid_shard_path_is_rejected_before_mutation(self):
        self.materialize(0)
        initial = read_local(self.local)
        index_path = os.path.join(
            self.pub.source_dir,
        ...
        with open(index_path) as file:
            index = json.load(file)
        index["weight_map"]["layer.a"] = "../outside.safetensors"
        with open(index_path, "w") as file:
            json.dump(index, file)

        with self.assertRaisesRegex(ValueError, "invalid delta shard path"):
            self.materialize(1)
        self.assertEqual(read_local(self.local), initial)
        self.assertEqual(disk_checkpoint._read_applied_version(self.local), 0)

    def test_checksum_algorithm_is_validated_before_mutation(self):
        self.materialize(0)
        initial = read_local(self.local)
        index_path = os.path.join(
            self.pub.source_dir,
        ...
        with open(index_path) as file:
            index = json.load(file)
        index["metadata"]["checksum_format"] = "unknown"
        with open(index_path, "w") as file:
            json.dump(index, file)
    ...
    def test_mixed_delta_encodings_share_the_target_read(self):
        ...
        self.assertEqual(stats["apply"]["operation"], "apply_delta_lineage")
        self.assertEqual(len(target_reads), 2)
        self.assert_at_version(4)
    ...
    def test_indexed_base_shard_may_link_to_huggingface_blob_store(self):
        ...
        self.assertEqual(disk_checkpoint._read_applied_version(self.local), 0)
        with (
            open(outside, "rb") as expected,
            open(os.path.join(self.local, "linked.safetensors"), "rb") as actual,
        ):
