...
// (inkling_ar_fused_decode.cuh). See CUSTOM_ALLREDUCE_JOURNAL.md for the design
// history (single-leader grid barrier) and the per-block variant.
//
...
__device__ __forceinline__ void grid_system_barrier(
    uint32_t* __restrict__ st, void* const* __restrict__ flag_ptrs, uint32_t rank, uint32_t idx,
    bool publish_writes) {
  ...
  if (gridDim.x > 1u) {
    if (threadIdx.x == 0) {
      ...
      asm volatile("atom.acq_rel.gpu.global.inc.u32 %0, [%1], %2;"
                   ...
                   : "memory");
      if (old == gridDim.x - 1u) {
        asm volatile("red.release.gpu.global.add.u32 [%0], %1;" ::"l"(release), "r"(1u)
                     : "memory");
      } else {
        uint32_t got;
        do {
...
// replays, like xepoch).
template <uint32_t kNumGPU>
__device__ __forceinline__ void block_system_barrier(
    uint32_t* __restrict__ st, void* const* __restrict__ flag_ptrs, uint32_t rank) {
  __shared__ uint32_t s_e;
  ...
  if (threadIdx.x < kNumGPU) {
    const uint32_t peer = threadIdx.x;
    uint32_t* remote = static_cast<uint32_t*>(flag_ptrs[peer]) + kNumGPU +
                       rank * kMaxBarrierBlocks + blockIdx.x;
    asm volatile("st.release.sys.global.u32 [%0], %1;" ::"l"(remote), "r"(e) : "memory");
    uint32_t* mine = static_cast<uint32_t*>(flag_ptrs[rank]) + kNumGPU +
                     peer * kMaxBarrierBlocks + blockIdx.x;
    uint32_t got;
    do {
      asm volatile("ld.acquire.sys.global.u32 %0, [%1];" : "=r"(got) : "l"(mine) : "memory");
