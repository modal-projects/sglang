...
class Publisher:
    ...
    def write_aggregate(self, version, stamp=None):
        """The aggregated delta the Aggregator maintains in a single in-place dir: base⊕v{version}
        as one xor (base_version 0), holding only tensors that differ from base. Every shard stamps
        the aggregate version in __metadata__ so a reader can reject a torn mid-update read; `stamp`
        forges a mismatched stamp for the torn-read test."""
        adir = os.path.join(self.source_dir, "aggregate")
        os.makedirs(adir, exist_ok=True)
        base, target = self.versions[0], self.versions[version]
        content, meta = {}, {"__version__": stamp or f"{version:06d}"}
        for name, new in target.items():
            if new == base[name]:
                continue
            diff = (
                np.frombuffer(new, dtype=np.uint8) ^ np.frombuffer(base[name], dtype=np.uint8)
            ).tobytes()
            content[name] = zstandard.ZstdCompressor().compress(diff)
            meta[name] = adler32_hex(new)
        write_safetensors(os.path.join(adir, self.SHARD), content, metadata=meta)
        with open(os.path.join(adir, "model.safetensors.index.json"), "w") as f:
            json.dump(
                {
                    "metadata": {
                        "version": f"{version:06d}",
                    ...
                    "weight_map": {name: self.SHARD for name in content},
                },
                f,
            )
        return adir
...
class PullTest(unittest.TestCase):
    ...
    def spy_pull(self):
        ...
        — enough to tell seed-base + aggregate-apply + tail apart from a full-chain reseed."""
        seeds, applies = [], []
    ...
    def test_aggregate_fast_forwards_then_folds_tail(self):
        # A fresh host seeds base, applies the aggregate (base⊕v3) in ONE xor to reach v3, then
        # applies only d4 — never replaying the d1..d3 chain.
        rng = np.random.default_rng(41)
        self.pub.publish_delta(3, {"layer.a": rng.integers(0, 256, 4096, dtype=np.uint8).tobytes()})
        self.pub.publish_delta(4, {"layer.b": rng.integers(0, 256, 2048, dtype=np.uint8).tobytes()})
        adir = self.pub.write_aggregate(3)
        with self.spy_pull() as (seeds, applies):
            self.pull(4)
        self.assert_at_version(4)
        self.assertEqual(seeds, [self.pub.base_dir], "seed base once")
        self.assertEqual(
            applies,
            [adir, os.path.join(self.pub.source_dir, "weight_v000004")],
            "apply the aggregate, then only the tail d4",
        )

    def test_aggregate_at_target_needs_no_tail(self):
        # aggregate at the target itself: seed base, apply the one aggregate, done (remaining==0).
        ...
        with self.spy_pull() as (seeds, applies):
            self.pull(3)
        self.assert_at_version(3)
        self.assertEqual((seeds, applies), ([self.pub.base_dir], [adir]))

    def test_running_host_ignores_aggregate(self):
        # A host past base (applied=2) applies the next delta incrementally and must NOT touch the
        # aggregate (whose base_version is 0) — it would discard warm state and mis-chain.
        self.pull(2)
        self.assert_at_version(2)
        ...
        with self.spy_pull() as (seeds, applies):
            self.pull(3)
        self.assert_at_version(3)
        self.assertEqual(seeds, [], "no reseed")
        self.assertEqual(applies, [os.path.join(self.pub.source_dir, "weight_v000003")])
        self.assertNotIn(adir, applies, "a running host must not apply the aggregate")

    def test_reseed_ignores_aggregate(self):
        # On reseed (recovery from corrupt local), replay comes from the pristine base+chain — never
        # the aggregate, which a separate writer may have left mid-update.
        ...
        with open(shard, "r+b") as f:
            f.seek(offset)
            f.write(bytes(16))  # silent local corruption -> the tail fold checksum-fails
        with self.spy_pull() as (seeds, applies):
            self.pull(3)
        self.assert_at_version(3)
        self.assertEqual(seeds, [self.pub.base_dir], "reseed must come from base")
        self.assertNotIn(adir, applies, "reseed must not apply the aggregate")

    def test_aggregate_ahead_of_target_not_used(self):
        # An aggregate newer than the target must be ignored: applying it would overshoot and desync
        # the caller's touched-name reload. The host seeds base and folds up to target.
        rng = np.random.default_rng(53)
        self.pub.publish_delta(3, {"layer.a": rng.integers(0, 256, 4096, dtype=np.uint8).tobytes()})
        adir = self.pub.write_aggregate(3)  # aggregate at v3, target below it
        with self.spy_pull() as (seeds, applies):
            self.pull(2)
        self.assert_at_version(2)
        self.assertEqual(seeds, [self.pub.base_dir])
        self.assertNotIn(adir, applies, "an aggregate ahead of target must not be applied")

    def test_torn_aggregate_is_skipped_not_applied(self):
        # A shard stamped a step behind the index == a mid-update read from an eventually-consistent
        # source. The per-shard checksums still self-verify, so only the version stamp catches it:
        # the host must skip the aggregate and fold the full chain from base, reaching the true target.
        rng = np.random.default_rng(61)
        self.pub.publish_delta(3, {"layer.a": rng.integers(0, 256, 4096, dtype=np.uint8).tobytes()})
        adir = self.pub.write_aggregate(3, stamp="000002")  # index says v3, shard stamped v2
        with self.spy_pull() as (seeds, applies):
            self.pull(3)
        self.assert_at_version(3)
        self.assertEqual(seeds, [self.pub.base_dir])
        self.assertNotIn(adir, applies, "a torn (stamp-mismatched) aggregate must not be applied")
