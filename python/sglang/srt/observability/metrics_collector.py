...
class QueueCount:
    ...
    # Requests retracted from decode waiting to resume.
    num_retracted: int = 0
...
class SchedulerStats:
    ...
    # Non-PD: prefill_queue_depth + decode_queue_depth == num_queue_reqs.total.
    # PD excludes completed prefill transfers and ordinary first-decode handoffs.
...
class SchedulerMetricsCollector(_StatLoggerDIMixin):
    def __init__(
        ...
        self.decode_queue_depth = Gauge(
            ...
            documentation="The number of waiting requests that were retracted from decode and are waiting to resume.",
            labelnames=labels.keys(),
            multiprocess_mode="mostrecent",
        )
