...
class ServingChatTestCase(unittest.TestCase):
    ...
    def test_convert_to_internal_request_rejects_sampling_mask_without_meta_info(
        self,
    ):
        req = ChatCompletionRequest(
            model="x",
        ...
        with self.assertRaisesRegex(
            ValueError, "return_sampling_mask requires return_meta_info=true"
        ):
            self.chat._convert_to_internal_request(req, self.fastapi_request)
    ...
    def test_kimi_k3_keeps_adjacent_image_placeholders_adjacent(self):
        image_data = []
        message = {
            "role": "user",
            "content": [
