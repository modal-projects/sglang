...
def _dflash_vocab_mask_allows_token(
    ...
    # Outlines-style masks are dense bool masks where True means "masked out".
    # xgrammar / llguidance masks are packed 32-token bitsets where a set bit
    # means "allowed".
...
def reset_dflash_vocab_mask_to_all_allowed(
    ...
    # Dense bool masks use True for masked-out tokens; packed bitmasks use a set
    # bit for allowed tokens.
...
def generate_dflash_linear_token_bitmask(
    *,
    reqs: List[Req],
    ...
    `draft_token_tail_cpu` contains candidates[:, 1:], i.e. the speculative
    tokens proposed after the already-accepted current token. Row j in the
    returned mask corresponds to target logits at verify position j:

      j == 0: grammar state before accepting any draft token in this block.
      j > 0: grammar state after accepting draft_token_tail_cpu[:, :j].

    The helper mutates request grammars only temporarily and rolls back before
    returning. Live grammar state is advanced later by v2 result processing.
    """
...
def validate_dflash_request(req: Req, enable_overlap: bool) -> Optional[str]:
    ...
    has_grammar_constraints = (
        req.sampling_params.json_schema is not None
        or req.sampling_params.regex is not None
        or req.sampling_params.ebnf is not None
        or req.sampling_params.structural_tag is not None
    )
    if has_grammar_constraints and not enable_overlap:
        return (
            "DFLASH speculative decoding does not support "
            "grammar-constrained decoding in spec v1 yet. "
            "Enable overlap scheduling to use DFLASH spec v2."
        )
