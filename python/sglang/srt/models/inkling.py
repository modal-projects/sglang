...
class InklingCausalLLM(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # DFLASH aux hidden states are packed directly as [tokens, K * hidden]
        # so the logits processor can store them without a second full-size
        # concat allocation.
        ...
        for layer in self.layers:
            ...
            if capture_aux and layer.layer_id in self._dflash_layers_to_capture:
                # Tapped layers have mlp_ar_fusable forced off (see
...
class InklingMTPLayer(nn.Module):
    ...
    def __init__(
        ...
        # MTP is derived from global_block, so always use is_local=False
