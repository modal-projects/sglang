...
class SchedulerRequestReceiver:
    ...
    def recv_requests(
        ...
        if self.recv_skipper is not None:
            if not self.recv_skipper.handle(self.get_last_batch()):
                # Reap plans and timeout aborts must reach every rank even when
                # socket polling is skipped, including subsequent PP stages.
    ...
    def _pull_raw_reqs(self, pull_sockets: bool = True) -> Optional[List]:
        """Keep reap planning and PP relay active when socket polling is skipped."""
        if self.ps.pp_rank == 0:
