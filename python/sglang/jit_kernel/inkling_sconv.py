...
def fused_causal_conv1d_update_decode(
    ...
    module = _jit_fused_decode_update_module(W, use_silu, use_residual, do_track, x.dtype)
    module.fused_decode_update(x, sconv_cache, cache_indices, cm, weight, y, tm, ti)
    return y


@cache_once
def _jit_draft_extend_sconv_module(w1: int, do_track: bool, dtype: torch.dtype) -> Module:
...
def fused_draft_extend_sconv_cache(
    ...
    module.draft_extend(
        hidden_states, sconv_cache, cache_indices, num_accepted_tokens,
        int(draft_token_num), cr, ts, mti,
    )
