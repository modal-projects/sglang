...
from sglang.jit_kernel.flash_attn.cute.softmax import SoftmaxSm100, apply_score_mod_inner
...
class FlashAttentionForwardSm100:
    ...
    def __init__(
        ...
        assert not (qk_blockscaled and self.q_sf_interleaved and is_varlen_q), (
            "if varlen q, can't have SFQ interleaved"
        )
        assert not (qk_blockscaled and not self.kv_sf_interleaved and self.use_tma_KV), (
    ...
    def __call__(
        ...
        if const_expr(mBias is not None):
            mBias = cute.make_tensor(mBias.iterator, cute.select(mBias.layout, mode=Q_layout_transpose))
        ...
        if const_expr(self.qk_blockscaled):
            # ((32,4),(32,4)):((16,4),(0,1))
            sf_atom = blockscaled_layout.BlockScaledBasicChunk(self.qk_sf_vec_size).layout
        ...
        if const_expr(self.v_dequant):
            sfv_atom = blockscaled_layout.BlockScaledBasicChunk(self.v_sf_vec_size).layout
        ...
        if const_expr(self.qk_blockscaled):
            ...
            self.tmem_sfk_offset = [
                self.tmem_sfq_offset[i] + self.num_sfq_tmem_cols for i in range(self.q_stage)
            ]
        self.ex2_emu_freq = 0
        ...
        if const_expr(self.qk_blockscaled):
            ...
            sSFK_layout = blockscaled_layout.make_smem_layout_sfb(
                self.tiled_mma_qk_sfk, self.mma_tiler_qk_sfk, self.qk_sf_vec_size, self.kv_stage
            )
        else:
            sSFQ_layout = None
            sSFK_layout = None
        if const_expr(self.v_dequant):
            ...
            sSFV_layout = blockscaled_layout.make_smem_layout_sfb(
                self.tiled_mma_pv_sfv, mma_tiler_pv_sfv, self.v_sf_vec_size, self.kv_stage
            )
        else:
            sSFV_layout = None
        ...
        if const_expr(self.use_tma_KV):
            ...
            tma_atom_V, mV = cute.nvgpu.make_tiled_tma_atom_B(
                tma_load_op,
                mV,
                cute.select(sVq_layout, mode=[0, 1, 2]) if const_expr(self.v_dequant)
        ...
        if const_expr(self.qk_blockscaled):
            ...
            num_load_threads = cute.arch.WARP_SIZE * len(self.load_warp_ids)
            thr_layout_SFQ = cute.make_ordered_layout((num_load_threads, 1), order=(0, 1))
        ...
        if const_expr(mBias is not None):
            ...
            bias_s2r_copy_atom = cute.make_copy_atom(
                cute.nvgpu.CopyUniversalOp(), self.bias_dtype, num_bits_per_copy=128,
            )
        ...
        if const_expr(self.overlap_sO_sQ and self.has_bias):
            ...
            # partials double sO) + fp8 KV (kv_stage at the formula's floor of
            # 2, small sK) + decode-shaped bias tiles (tile_bias < 128, tiny
            # sBias), sQ + sK + sBias can be SMALLER than sO, and the
            # unpredicated sO stores then land past the smem allocation ->
            # cudaErrorIllegalAddress. Pad sQ by the shortfall so the struct
            # always covers sO.
        ...
        class SharedStorage:
            ...
            mbar_v_upcast: cute.struct.MemRange[Int64, self.v_mma_stage * 2 * use_vq_mbar]
            ...
            sVq: cute.struct.Align[
                cute.struct.MemRange[
                    self.v_dtype if const_expr(self.v_dequant) else cutlass.Float8E4M3FN,
                    sVq_size,
                ],
                self.buffer_align_bytes,
            ]
            sV_dequant: cute.struct.Align[
                cute.struct.MemRange[
                    self.v_mma_dtype if const_expr(self.v_dequant) else cutlass.BFloat16,
                    sV_dequant_size,
                ],
                self.buffer_align_bytes,
            ]
            sSFQ: cute.struct.Align[
                cute.struct.MemRange[
                    self.sfq_dtype if const_expr(self.qk_blockscaled) else cutlass.Float8E8M0FNU,
                    sSFQ_size,
                ],
                self.buffer_align_bytes,
            ]
            sSFK: cute.struct.Align[
                cute.struct.MemRange[
                    self.sfk_dtype if const_expr(self.qk_blockscaled) else cutlass.Float8E8M0FNU,
                    sSFK_size,
                ],
                self.buffer_align_bytes,
            ]
            sSFV: cute.struct.Align[
                cute.struct.MemRange[
                    self.sfv_dtype if const_expr(self.v_dequant) else cutlass.Float8E8M0FNU,
                    sSFV_size,
                ],
                self.buffer_align_bytes,
            ]
    ...
    def kernel(
        ...
        if warp_idx == 0:
            for tma_atom in (
                ...
                tma_atom_SFQ, tma_atom_SFK, tma_atom_SFV,
            ):
        ...
        if const_expr(self.v_dequant):
            ...
            sV = cute.make_tensor(
                cute.recast_ptr(sK.iterator, sV_layout.inner, self.v_mma_dtype), sV_layout.outer
            )
        if const_expr(self.qk_blockscaled):
        ...
        thr_mma_qk_sfk = (
            tiled_mma_qk_sfk.get_slice(mma_tile_coord_v) if const_expr(self.qk_blockscaled) else None
        )
        thr_mma_pv_sfv = (
            tiled_mma_pv_sfv.get_slice(mma_tile_coord_v) if const_expr(self.v_dequant) else None
        )
        thr_mma_pv_vq = (
            tiled_mma_pv_vq.get_slice(mma_tile_coord_v) if const_expr(self.v_dequant) else None
        )
    ...
    def load(
        ...
        while work_tile.is_valid_tile:
            ...
            load_SFQ_fn = None
            if const_expr(self.use_cpasync_to_load_sfq):
                tiler_gQ_sf = ((self.mma_tiler_qk[0] * self.q_stage), self.head_dim_padded // self.qk_sf_vec_size)
                ...
                tiler_gQ_sfa = ((self.mma_tiler_qk[0] * self.q_stage), self.head_dim_padded // self.qk_sf_vec_size)
                mSFQ_cur = seqlen.offset_batch_Q(mSFQ, batch_idx, dim=3)[None, None, head_idx]
            ...
            if const_expr(mPageTable is None):
                if const_expr(not seqlen.has_cu_seqlens_k):
                    ...
                    if const_expr(self.qk_blockscaled):
                        mSFK_cur = cute.domain_offset((seqlen.offset_k, 0), mSFK[None, None, head_idx_kv])
                    if const_expr(self.v_dequant):
                        # SFV is token-major (like SFK): offset the token axis.
                        mSFV_cur = cute.domain_offset((seqlen.offset_k, 0), mSFV[None, None, head_idx_kv])
                ...
                if const_expr(self.qk_blockscaled):
                    gSFK = cute.local_tile(mSFK_cur, cute.select(self.mma_tiler_qk_sfk, mode=[1, 2]), (None, 0))
                if const_expr(self.v_dequant):
                    gSFV = cute.local_tile(mSFV_cur, cute.select(self.mma_tiler_pv_sfv, mode=[1, 2]), (None, 0))
            else:
                ...
                if const_expr(self.v_dequant):
                    gSFV = cute.local_tile(
                        mSFV_cur, cute.select(self.mma_tiler_pv_sfv, mode=[1, 2]), (None, 0, None)
                    )
            ...
            if const_expr(self.use_tma_KV):
                ...
                if const_expr(self.qk_blockscaled):
                    ...
                    tKsSFK, tKgSFK = cpasync.tma_partition(
                        ...
                        cute.group_modes(sSFK, 0, 3), cute.group_modes(tSgSFK, 0, 3),
                    )
                ...
                if const_expr(self.v_dequant):
                    ...
                    tVsSFV, tVgSFV = cpasync.tma_partition(
                        tma_atom_SFV, 0, cute.make_layout(1),
                        cute.group_modes(sSFV, 0, 3), cute.group_modes(tOgSFV, 0, 3),
                    )
            ...
            if const_expr(not self.use_block_sparsity):
                ...
                if const_expr(tma_atom_bias is not None):
                    ...
                    _, n_block_max_abs0 = block_info.get_n_block_min_max(
                        seqlen, self.q_stage * m_block, split_idx, num_splits,
                    ...
                    dummy_first_bias_load = (
                        bias_idx_offset0 < 0 and const_expr(self.q_stage == 2)
                    )
    ...
    def dequant_v(
        ...
        tiled_copy_s2r = copy_utils.tiled_copy_2d(
            self.v_dtype, threads_per_row=4, num_threads=num_threads, num_copy_elems=num_load_elems,
        )
        tiled_copy_r2s = copy_utils.tiled_copy_2d(
            self.v_mma_dtype, threads_per_row=4, num_threads=num_threads, num_copy_elems=num_store_elems,
        )
        ...
        cV = cute.composition(cV, cute.make_layout(sVq_.shape[:2], stride=sVq_.stride[:2]))
        ...
        pipeline_vq.consumer_wait_w_index_phase(vq_consumer_state.index, vq_consumer_state.phase)
    ...
    def softmax_loop(
        ...
        tmem_store_atom = cute.make_copy_atom(
            tcgen05.copy.St32x32bOp(
                tcgen05.copy.Repetition(8 if const_expr(self.v_mma_dtype.width == 8) else 16)
            ),
            Float32,
        )
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(self.has_bias):
                _, n_block_max_abs_local = block_info.get_n_block_min_max(
                    seqlen, self.q_stage * m_block + stage, split_idx, num_splits,
                    half_tile_m=self.q_stage > 1, absolute=True,
                )
                ...
                if const_expr(not self.is_split_kv) or tile_block_count > Int32(0):
                    ...
                    if const_expr(self.is_causal or self.is_local or self.has_bias):
                        if const_expr(self.has_bias):
                            ...
                            n_block_min_bias_emu_off = block_info.get_n_block_min_causal_local_mask(
                                seqlen, m_block, n_block_min
                            )
                            n_block_left_bias_emu_off = block_info.get_n_block_min_before_local_mask(
                                seqlen, m_block, n_block_min
                            )
                        else:
    ...
    def softmax_step(
        ...
        if const_expr(self.has_bias and apply_bias):
            ...
            tBrS = cute.make_tensor(
                tSrS_t2r.iterator, cute.make_fragment_like(tS2RsBias[None, None, None, 0].layout)
            )
    ...
    def correction_loop(
        ...
        while work_tile.is_valid_tile:
            ...
            max_offset = Float32(8.0) if cutlass.const_expr(self.v_mma_dtype.width == 8) else Float32(0.0)
            max_offset_scale = (
                Float32(256.0) if cutlass.const_expr(self.v_mma_dtype.width == 8) else Float32(1.0)
            )
            seqlen = SeqlenInfoCls(batch_idx)
            n_block_min, n_block_max = block_info.get_n_block_min_max(seqlen, m_block, split_idx, num_splits)
    ...
    def cpasync_load_SFQ(
        ...
        sSFQ_cpt_shape_mdp = (self.m_block_size, self.head_dim_padded // self.qk_sf_vec_size, self.q_stage)
        sSFQ_cpt_layout_mdp = cute.make_ordered_layout(sSFQ_cpt_shape_mdp, order=(0, 1, 2))
        ...
        cSFQ = cute.make_identity_tensor((self.m_block_size, self.head_dim_padded // self.qk_sf_vec_size))
    ...
    def load_KV(
        ...
        if const_expr(self.use_tma_KV):
            ...
            if const_expr(blockscaled):
                ...
                cute.copy(
                    tma_atom_sf, tXgSFX_cur, tXsSFX_cur,
            ...
            if const_expr(sSFX is not None):
                paged_kv_manager.load_sf_KV(block, sSFX[None, None, None, sf_stage], K_or_V)
            cute.arch.cp_async_commit_group()
