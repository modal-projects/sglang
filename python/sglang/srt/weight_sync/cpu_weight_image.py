...
class CPUWeightImage:
    ...
    def __init__(self, model: torch.nn.Module):
        ...
        if len(devices) != 1:
            raise RuntimeError(
                f"a rank-local CPU weight image requires one CUDA device: {devices}"
            )
    ...
    def storage_image_bytes(self, tensor: torch.Tensor) -> torch.Tensor:
        """Return an isolated CPU view for one live weight storage.
    ...
    def validate_commit(self, target_version: int) -> None:
        ...
        if not self.registered:
            raise RuntimeError(
                "staged CPU image is not registered; registration must finish "
    ...
    def validate_against_active(self) -> dict[str, int | float | str]:
        ...
        if mismatches:
            ...
            raise RuntimeError(
                "CPU weight image does not reproduce the active model: "
