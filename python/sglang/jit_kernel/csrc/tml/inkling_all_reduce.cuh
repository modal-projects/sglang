// Self-contained two-shot (reduce-scatter + all-gather) all-reduce over a torch
// symmetric-memory buffer, for the Inkling (Moonrise) tensor-parallel reductions.
//
...
// vec count.
template <typename DType, uint32_t kNumGPU>
__device__ __forceinline__ uint32_t slice_setup(
    DType* (&input)[kNumGPU], void* const* peer_ptrs, uint32_t rank, uint32_t num_items) {
  using Trait = InklingAllReduceTrait<DType, kNumGPU>;
  const uint2 slice = rank_vec_slice<DType, kNumGPU>(rank, num_items);
  const uint32_t base = slice.x * Trait::kElemsPerVec;
#pragma unroll
  for (uint32_t i = 0; i < kNumGPU; ++i) input[i] = static_cast<DType*>(peer_ptrs[i]) + base;
  return slice.y;  // local vec count
}

template <typename DType, uint32_t kNumGPU>
__device__ __forceinline__ void two_shot_reduce_local(
    DType* (&input)[kNumGPU], uint32_t local_vecs) {
  using namespace device;
  using Trait = InklingAllReduceTrait<DType, kNumGPU>;
  ...
  for (uint32_t v = blockIdx.x * blockDim.x + threadIdx.x; v < local_vecs; v += stride) {
    Storage s[kNumGPU];
#pragma unroll
    for (uint32_t i = 0; i < kNumGPU; ++i) s[i].load(input[i], v);
    Storage result;
#pragma unroll
...
#pragma unroll
    for (uint32_t i = 0; i < kNumGPU; ++i) result.store(input[i], v);
  }
}
...
__global__ __launch_bounds__(1024, 1) void inkling_two_shot_all_reduce_fused_kernel(
  ...
  DType* input[kNumGPU];
  const uint32_t local_vecs = slice_setup<DType, kNumGPU>(input, peer_ptrs, rank, num_items);
  inkling_ar::grid_system_barrier<kNumGPU>(state, flag_ptrs, rank, 0, /*publish_writes=*/false);  // ENTRY: producers done + visible
  two_shot_reduce_local<DType, kNumGPU>(input, local_vecs);
  inkling_ar::grid_system_barrier<kNumGPU>(state, flag_ptrs, rank, 1, /*publish_writes=*/true);  // EXIT: broadcasts done + visible
}
...
__global__ __launch_bounds__(1024, 1) void inkling_multimem_push_oneshot_kernel(
  ...
  for (uint32_t v = blockIdx.x * blockDim.x + threadIdx.x; v < total_vec; v += stride) {
    Storage s[kNumGPU];
#pragma unroll
    for (uint32_t i = 0; i < kNumGPU; ++i) s[i].load(stage_ptr + i * num_items, v);
    Storage result;
#pragma unroll
    for (uint32_t j = 0; j < kVecSize; ++j) {
      fp32x2_t acc = cast<fp32x2_t>(s[0][j]);
...
__global__ __launch_bounds__(1024, 1) void inkling_multimem_full_oneshot_kernel(
  ...
  inkling_ar::grid_system_barrier<kNumGPU>(state, flag_ptrs, rank, 0, /*publish_writes=*/false);  // ENTRY only (single barrier)
  const uint32_t stride = gridDim.x * blockDim.x;
  for (uint32_t v = blockIdx.x * blockDim.x + threadIdx.x; v < total_vec; v += stride) {
...
void inkling_two_shot_all_reduce(
  ...
  LaunchKernel(num_blocks, 1024u, stream)(
      ...
      reinterpret_cast<void* const*>(peer_ptrs_dev), static_cast<uint32_t>(rank), n);
}
...
void inkling_two_shot_all_reduce_fused(
  ...
  const uint32_t num_blocks = nb_override > 0
                                  ? min(static_cast<uint32_t>(nb_override), cap)
...
void inkling_multimem_one_shot_fused(
  ...
  const auto device = local_buffer.device();
  const auto kernel = per_block_barrier
                          ? inkling_multimem_one_shot_fused_kernel<DType, kNumGPU, true>
...
void inkling_multimem_push_oneshot(
  ...
  const auto kernel = per_block_barrier
                          ? inkling_multimem_push_oneshot_kernel<DType, kNumGPU, true>
