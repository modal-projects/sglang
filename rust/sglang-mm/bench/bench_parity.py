...
def encode(arr: np.ndarray, fmt: str) -> bytes:
    buf = io.BytesIO()
    Image.fromarray(arr).save(buf, format=fmt, **({"quality": 90} if fmt == "JPEG" else {}))
    return buf.getvalue()
...
def parity_b():
    ...
    for fmt in ["PNG", "JPEG"]:
        data = encode(arr, fmt)
        ref = _encode_image_bytes(data, patch_size=PS, rescale_image_frac=None, rescale_image_max_upscaled_long_edge=None)
        got = rs_decode_patchify(data)
        got2 = got.view(got.shape[0], 1, PS, PS, 3).expand(-1, 2, -1, -1, -1)
        if torch.equal(ref.contiguous().view(torch.uint16), got2.contiguous().view(torch.uint16)):
...
def bench():
    ...
    _encode_image_bytes(jpeg, patch_size=PS, rescale_image_frac=None, rescale_image_max_upscaled_long_edge=None)
    ...
    w_py, c_py = run("python (PIL + numba + bf16 cast)", lambda: _encode_image_bytes(jpeg, patch_size=PS, rescale_image_frac=None, rescale_image_max_upscaled_long_edge=None))
    w_rs, c_rs = run("rust  decode_patchify", lambda: rs_decode_patchify(jpeg))
    w_rb, c_rb = run(
        "rust  decode_patchify_batch (5 imgs/call)",
        lambda: sglang.srt.multimodal._core.inkling.decode_patchify_batch([jpeg] * 5, PS),
        iters=max(n // 5, 5),
        images_per_call=5,
    )
