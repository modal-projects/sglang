...
class _ModuleQuantMethod(torch.nn.Module, QuantizeMethodBase):
...
def test_weight_plan_deduplicates_aliases_and_requires_explicit_derived_state():
    ...
    segments, image_nbytes = build_rank_weight_image_plan(
        model,
        device_type="cpu",
    )
...
def test_weight_plan_routes_module_quantization_hooks_through_the_owner():
    model = torch.nn.Module()
    model.weight = torch.nn.Parameter(torch.ones(2))
    model.quant_derived = torch.arange(3)
    model.quant_method = _ModuleQuantMethod()
...
def test_staging_state_fails_closed_until_a_complete_image_is_ready():
    image = RankWeightImage.__new__(RankWeightImage)
