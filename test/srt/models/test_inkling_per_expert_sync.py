...
class _FakeModel:
    ...
    _load_per_expert_param = inkling_mod.InklingForConditionalGeneration._load_per_expert_param
...
class TestPerExpertSync(unittest.TestCase):
    ...
    def _run_rank(self, full, ep_size, ep_rank, tp_size, tp_rank, *, interleaved, lora_layout):
        ...
        exp_w13, exp_w2 = _expected_stacks(full, ep_size, ep_rank, tp_size, tp_rank, contiguous)
    ...
    def test_ep_tp_grid_interleaved(self):
        ...
        for ep_rank in range(4):
            for tp_rank in range(2):
                self._run_rank(full, 4, ep_rank, 2, tp_rank, interleaved=True, lora_layout=False)
    ...
    def test_ep_tp_grid_contiguous_layouts(self):
        ...
        for interleaved, lora_layout in ((True, True), (False, False)):
            for ep_rank in range(2):
                self._run_rank(
                    full, 2, ep_rank, 2, 1, interleaved=interleaved, lora_layout=lora_layout
                )
