"""Canonical safetensors checkpoints backed by host memory or local storage."""
...
def _checkpoint_manifest(
    ...
    for filename in filenames:
        ...
        # Hugging Face cache snapshots conventionally expose checkpoint blobs
        # through symlinks. The manifest name must stay inside the checkpoint
        # namespace, but the file it names may resolve into the cache's blob
        # store.
        path = root / relative
        if not path.is_file():
...
class CanonicalCheckpoint:
        ...
        *,
        ...
        if storage == "memory":
            self._storage = HostLocalSharedBuffer(
                ...
                # Local workers are commonly pinned to different NUMA nodes.
                # Spread this shared, read-mostly source across all allowed nodes.
    ...
    def finish_update(self, target_version: int) -> None:
        if not self.updating:
            raise RuntimeError("canonical checkpoint update is not in progress")
        if target_version != self._target_version:
            raise ValueError(
                "canonical checkpoint target changed during update: "
                f"expected={self._target_version} actual={target_version}"
            )
    ...
    def release_cached_pages(self) -> dict[str, int | float]:
        """Release clean file-backed pages after consumers finish reading."""
