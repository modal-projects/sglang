...
def compare(tag, images, expect_exact, rescale=False):
    ...
    exact = torch.equal(a.contiguous().view(torch.uint16), b.contiguous().view(torch.uint16))
    if expect_exact:
        assert exact, f"{tag}: expected bit-exact"
        print(f"  {tag}: bit-exact=True shape={tuple(a.shape)}")
    else:
        d = (a.float() - b.float()).abs()
        print(f"  {tag}: bit-exact={exact} max_abs={d.max():.6f} shape={tuple(a.shape)}")
...
compare("5x PNG batch", [encode(a, "PNG") for a in [arr1, arr2, arr3, arr1, arr2]], expect_exact=True)
compare("mixed JPEG/PNG batch", [encode(arr1, "JPEG"), encode(arr2, "PNG")], expect_exact=False)
...
compare("single JPEG rescaled", [encode(arr1, "JPEG")], expect_exact=False, rescale=True)
compare(
    "3x mixed rescaled",
