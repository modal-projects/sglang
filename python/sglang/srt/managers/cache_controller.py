...
class HiCacheController:
    ...
    def __init__(
        ...
        # HiCache reads it back on write_stream. The two event dependencies below
    ...
    def start_writing(self) -> None:
        ...
        if self.has_draft:
            # The draft pool may use staged write-back even when the target is a
            # non-src MLA dummy pool. Resolve its indices independently: the
            # staged kernel requires CPU destination indices, whereas the dummy
            # target's fallback path moves them to CUDA.
        ...
        with device_module.stream(self.write_stream):
            ...
            # D2H must not read cache slots before the previous model forward
            # has finished producing them. This matters in scheduler overlap
            # mode, where the current/schedule stream is not the producer.
