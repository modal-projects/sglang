...
class PrefillAdder:
    ...
    def _memory_budget_block_cause(self, needed_full_tokens: int = 0) -> str:
        """Attribute a memory-budget rejection to the FULL KV or the SWA budget.

        A best-effort re-check, not a second gate. FULL is blamed when its
        budget cannot cover ``needed_full_tokens`` (0 means "any progress"),
        otherwise the rejection came from an SWA-specific check. The shared
        FULL/SWA budget has no separate SWA pool to blame, so it reads as FULL.
        """
        budget = self.memory_budget
        if self.is_hybrid_swa and not isinstance(budget, SharedSWAPrefillBudget):
            remaining_full = min(budget.remaining_total, budget.remaining_current)
            if remaining_full > max(needed_full_tokens, 0):
                return AdmissionBlockCause.SWA_TOKENS
    ...
    def budget_state(self):
        if not self.memory_budget.has_capacity():
            return self._stop(AddReqResult.NO_TOKEN, self._memory_budget_block_cause())
    ...
    def add_one_req_ignore_eos(self, req: Req):
        ...
        if not fits:
            return self._stop(
                AddReqResult.NO_TOKEN, self._memory_budget_block_cause(paged_input)
            )
    ...
    def _select_prefill_admission(
        ...
        *,
        ...
        if not can_admit:
            return self._stop(
                AddReqResult.NO_TOKEN, self._memory_budget_block_cause(total_tokens)
            )
