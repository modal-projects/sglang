...
class StreamingSession(BasePrefixCache):
    ...
    def try_cache_finished_req(
        ...
        # occurs on terminal releases and cannot identify a retract.
        if is_retract:
            self._release_turn_kv(slot, req, session_id)
            return True
    ...
    def _release_turn_kv(
        ...
        kv = req.detach_kv()
        # Host backups belong to the live request, not the discarded session row.
        req.kv.retraction_backup = kv.retraction_backup
