...
from sglang.tml.kernels.comm import get_ar_buffer, symm_mem_all_reduce
from sglang.tml.kernels.gate_topk import gate_topk
from sglang.tml.kernels.sigmoid_gate_topk_renorm import (
    sigmoid_gate_topk_renorm,
)
from sglang.tml.kernels.inkling_moe import (
...
from sglang.tml.layers.dense_mlp import InklingBatchDenseMLP, InklingDenseMLP
from sglang.tml.layers.util import lora_compatible_layout_enabled

_INKLING_FUSED_GATE_OUT_FEATURES = 258
# Pad the gate's expert (output-feature) dim up to a multiple of 8 so the
# bf16->fp32 GEMM stays on the fast aligned cuBLAS path -- an unaligned N=258
# is up to ~70% slower. The padding is baked into the weight once at load
# (see _load_gate_weight_padded), so forward pays no per-step F.pad.
_INKLING_FUSED_GATE_OUT_FEATURES_PADDED = (_INKLING_FUSED_GATE_OUT_FEATURES + 7) // 8 * 8
...
def inkling_fused_gate_linear_with_fp32_out(
    ...
    return torch.mm(input, weight.T, out_dtype=torch.float32)[
...
def linear_with_fp32_out(
    ...
    if bias is None:
        ...
        out = torch.addmm(bias, flat_input, weight.T, out_dtype=torch.float32)
...
def run_moe_preprocess(
    ...
    ) = compute_grouped_gemm_metadata(reorder_topk_ids, num_experts)
    return (
        src2dst,
        num_tokens_per_expert,
        expert_token_offs,
...
class InklingSharedFusedMoE(FusedMoE):
    ...
    def __init__(
        ...
        # separately at load time in models/tml.py.
...
def _build_inkling_shared_experts(
    *,
    ...
    if shared_expert_sink:
        if envs.SGLANG_OPT_USE_INKLING_SHARED_FUSED_MOE.get():
...
class InklingMoE(nn.Module):
    def __init__(
        ...
        # Packed topk is only for the quantized SRT apply path; moe_tp_forward and the
        # MoE-LoRA dispatch need standard topk (the LoRA kernel packs internally).
    ...
    def _forward_routed(
        ...
        if (
            ...
            # delta, so bf16 layers run the stock forward (w13 de-interleaved at load).
    ...
    def _forward_shared(
        ...
        if self.shared_expert_sink:
            # Key on InklingBatchDenseMLP (never LoRA-wrapped), not `not isinstance(...,
            # InklingSharedFusedMoE)`: the LoRA-wrapped sink must take the FusedMoE call below.
            if isinstance(self.shared_experts, InklingBatchDenseMLP):
                # Legacy path (SGLANG_OPT_USE_INKLING_SHARED_FUSED_MOE=False): batched
                # bmm sink, takes gammas directly.
            ...
            if (
                ...
                self._shared_topk_ids = torch.arange(
                    self.n_shared_experts, dtype=torch.int32, device=hidden_states.device
                )
            topk_output = StandardTopKOutput(
    ...
    def forward(
        ...
        """With ``reduce=False`` the TP all-reduce is skipped and the LOCAL
        partial sums (routed + shared) are returned -- the caller takes over
        the reduction (e.g. the fused decode {AR -> mlp_sconv -> attn_norm}
        kernel, see tml/kernels/comm.py::ar_sconv_norm_fused)."""
        del forward_batch
        topk_weights, topk_ids, shared_gammas, packed_topk_ids = self.gate(x)

        lora_on = lora_compatible_layout_enabled()
        if lora_on:
        ...
        use_two_stream = (
            ...
            # Sink LoRA shrink buffers must alloc on the consumer stream (WAR -> IMA).
            and (not lora_on or lora_envs.SGLANG_OPT_LORA_OVERLAP_MAIN_ALLOC.get())
        )
        if use_two_stream:
        ...
        if not reduce:
            return out + shared_out if shared_out is not None else out
