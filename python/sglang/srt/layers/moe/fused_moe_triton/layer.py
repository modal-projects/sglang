...
def _prepared_weight_copy_workers() -> int:
...
class FusedMoE(torch.nn.Module):
    ...
    def supports_batched_weight_loading(self) -> bool:
        """Return whether this loader is a pure set of independent copies."""
    ...
    def _execute_prepared_weight_copy_pairs(
        self,
        ...
        *,
        ...
        if copy_pairs:
            pairs_by_storage = {}
            for pair_index, (target, _) in enumerate(copy_pairs):
            ...
            for key, entries in pairs_by_storage.items():
                if any(not contiguous for _, contiguous, _, _ in entries):
                    ordered_storage_keys.add(key)
                    continue
                ranges = sorted(
                    (begin, end) for _, _, begin, end in entries if begin < end
                )
                if any(
                ...
                if copy_workers == 1:
                    ...
                    for pair in weighted_pairs:
                        ...
                        chunk_bytes[chunk_index] += (
                            pair[0].numel() * pair[0].element_size()
                        )
                    chunks = [chunk for chunk in chunks if chunk]
                    futures = [executor.submit(copy_group, chunk) for chunk in chunks]
                    for future in futures:
                        future.result()
            for target, source in ordered_pairs:
                target.copy_(source)

    def weight_loader_batch(
        self,
        ...
        *,
        ...
        try:
            ...
            self._execute_prepared_weight_copy_pairs(
                copy_pairs,
                executor=executor,
            )
        finally:
    ...
    def _load_per_tensor_weight_scale(
        ...
        if shard_id in ("w1", "w3"):
            ...
            if self.moe_runner_config.is_gated:
                self._copy_loaded_weight(
                    param_data[expert_id][idx],
                    loaded_weight,
                )
            else:
