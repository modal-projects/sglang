...
@dsl_user_op
def cp_async_4(
    dst_smem: cutlass.Int32,
    src_gmem: cutlass.Int64,
    src_size: cutlass.Int32,
    *,
    loc=None,
    ip=None,
) -> None:
    """4-byte cp.async.ca with the src-size operand: src_size=4 copies, 0
    zero-fills the destination (used for out-of-range scale rows -- e8m0 0x00
    is 2^-127, benign). Completion rides the surrounding
    cp_async_commit_group + mbarrier arrive like every other cp.async."""
    llvm.inline_asm(
        None,
        [
            cutlass.Int32(dst_smem).ir_value(loc=loc, ip=ip),
            cutlass.Int64(src_gmem).ir_value(loc=loc, ip=ip),
            cutlass.Int32(src_size).ir_value(loc=loc, ip=ip),
        ],
...
@dsl_user_op
def prmt_b32(
    a: cutlass.Uint32, b: cutlass.Uint32, code: cutlass.Uint32, *, loc=None, ip=None
) -> cutlass.Uint32:
    """PTX prmt.b32: permute bytes of the 8-byte pool {a, b} per selector nibbles."""
    return cutlass.Uint32(
        llvm.inline_asm(
            T.i32(),
            [
                ...
                cutlass.Uint32(code).ir_value(loc=loc, ip=ip),
            ],
            "prmt.b32 $0, $1, $2, $3;",
            "=r,r,r,r",
            has_side_effects=False,
...
def mul_bf16x2(
    ...
    """PTX mul.rn.bf16x2 on two packed-bf16 pairs held in u32 registers."""
    return cutlass.Uint32(
        llvm.inline_asm(
            T.i32(),
            [
...
@dsl_user_op
def cvt_bf16x2_ue8m0x2(a: cutlass.Int16, *, loc=None, ip=None) -> cutlass.Int32:
    """
    cvt.rn.bf16x2.ue8m0x2  d, a;

    Converts a packed ue8m0x2 value (two UE8M0 block-scale exponents in a
    16-bit register) to a packed bf16x2 (two BF16s in a 32-bit register).

    rn = round-to-nearest-even (only rounding mode supported for this direction).
    No .satfinite needed — UE8M0 values always map to representable BF16s.
    """
    return cutlass.Int32(
        llvm.inline_asm(
            T.i32(),
            [cutlass.Int16(a).ir_value(loc=loc, ip=ip)],
            "cvt.rn.bf16x2.ue8m0x2 $0, $1;",
            "=r,h",  # r = 32-bit dest (bf16x2), h = 16-bit src (ue8m0x2)
            has_side_effects=False,
            is_align_stack=False,
            asm_dialect=llvm.AsmDialect.AD_ATT,
        )
...
@cute.jit
def cvt_tensor_ue8m0_to_bf16(
    scales: cute.Tensor,
    scales_out: cute.Tensor,
):
    n = cute.size(scales)
    assert n % 2 == 0
    assert cute.size(scales_out) == n
    ...
    for i in cutlass.range_constexpr(n // 2):
        scales_out_x2[i] = cvt_bf16x2_ue8m0x2(scales_x2[i])
