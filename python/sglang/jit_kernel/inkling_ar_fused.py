...
def inkling_ar_sconv_norm(
    ...
    module.ar_sconv_norm(
        in_partial, residual_in, residual_out, hs_out, norm_weight, float(eps),
        sconv_cache, cache_indices, cache_mask.reshape(-1), conv_weight, tm, ti,
        mc_stage_ptr, local_stage_ptr, flag_ptrs_dev, state_ptr, rank,
        int(enable_pdl), int(vecs_per_thread),
    )
...
def inkling_ar_sconv_norm_verify(
    ...
    module.ar_sconv_norm_verify(
        in_partial, residual_in, residual_out, hs_out, norm_weight, float(eps),
        sconv_cache, cache_indices.to(torch.int32), cache_mask, conv_weight,
        inter_out, int(draft_token_num),
        mc_stage_ptr, local_stage_ptr, flag_ptrs_dev, state_ptr, rank,
        int(enable_pdl),
    )
