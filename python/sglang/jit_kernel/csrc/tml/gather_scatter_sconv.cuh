// Inkling sconv: fused gather->scatter into sconv_cache -- CUDA-JIT port of the Triton
// kernel `_fused_gather_scatter_to_sconv_cache_kernel` in tml/layers/sconv.py (opt5).
//
...
#pragma unroll
  for (int w = 0; w < W1; ++w) {
    ...
    const __nv_bfloat162 v =
        *reinterpret_cast<const __nv_bfloat162*>(&hp[src_t * p.hs_stride_t + c0]);
