...
def resolve_hicache_dcp_compatibility(server_args: Any):
    ...
    if cfg.speculative_algorithm not in (None, "DFLASH", "DSPARK"):
        raise NotImplementedError(
            "HiCache with --dcp-size > 1 only supports chain-draft DFLASH/DSPARK "
            "speculative decoding; other draft-model host pools have no DCP "
            "index translation."
        )
