...
def rs_resize(arr: np.ndarray, tw: int, th: int) -> np.ndarray:
    return sglang.srt.multimodal._core.inkling.resize_rgb(arr, tw, th).reshape(th, tw, 3)
...
@pytest.mark.parametrize("h,w,th,tw", CASES, ids=[f"{h}x{w}->{th}x{tw}" for h, w, th, tw in CASES])
...
def test_scaled_dims_sweep():
    ...
    for frac, cap in [(2.0, 2048), (1.5, 2048), (3.0, None), (None, None), (2.0, 1)]:
        for w, h in sizes:
            assert sglang.srt.multimodal._core.inkling.scaled_dims(w, h, frac, cap) == py_scaled_dims(w, h, frac, cap), (
                w, h, frac, cap,
            )
...
def test_decode_patchify_rescaled_matches_pil_pipeline():
    ...
    h, w, bits = sglang.srt.multimodal._core.inkling.decode_patchify(buf.getvalue(), 40, 2.0, 2048)
