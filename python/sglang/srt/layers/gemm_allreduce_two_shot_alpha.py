# Vendored from flashinfer 0.6.11.post1 flashinfer/cute_dsl/gemm_allreduce_two_shot.py
# (byte-identical to the lmsysorg/sglang:v0.5.12 image copy) with ONE modification:
# a runtime per-launch fp32 `alpha` scale applied to the fp32 accumulator BEFORE the
# C-dtype cast in the epilogue (both TMA-store and SIMT-store paths). This is required
# for fp8 e4m3 A/B with per-tensor STATIC scales at TP: alpha = input_scale *
# weight_scale differs PER RANK (each rank quantizes its own K-shard), so the scale
# must be folded into the per-rank partial before the two-shot multimem reduce.
# alpha=1.0 preserves the original numerics exactly (IEEE mul identity).
            ...
            while work_tile.is_valid_tile:
                ...
                for subtile_idx in cutlass.range(subtile_cnt):
                    ...
                    if cutlass.const_expr(self.use_tma_store):
                        ...
                        acc_vec = acc_vec * alpha  # runtime per-rank scale (fp32)
