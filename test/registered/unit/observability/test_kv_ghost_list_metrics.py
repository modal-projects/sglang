...
class TestKVGhostList(unittest.TestCase):
    def test_hit_pops_and_reports_delay(self):
        ...
        self.assertEqual(ghosts.match([1, 2, 3], now=15.0), [GhostHit(5.0, 3)])
    ...
    def test_partial_and_grouped_hits(self):
        ...
        self.assertEqual(
            ...
            [GhostHit(20.0, 2), GhostHit(10.0, 1)],
        )
    ...
    def test_ttl_expires_on_record_and_on_match(self):
        ...
        self.assertEqual(ghosts.match([4], now=17.0), [GhostHit(1.0, 1)])
    ...
    def test_capacity_drops_oldest_first(self):
        ...
        self.assertEqual(
            ghosts.match([2, 3], now=4.0), [GhostHit(2.0, 1), GhostHit(1.0, 1)]
        )
    ...
    def test_re_record_refreshes_time_and_position(self):
        ...
        self.assertEqual(ghosts.match([1], now=5.0), [GhostHit(2.0, 1)])
...
class TestCollectorContract(unittest.TestCase):
    ...
    def test_reinsert_observation_and_token_bucket(self):
        c = self.collector
        c.observe_kv_reinsert_after_evict(90.0, 128)
        self.assertEqual(
        ...
        c.observe_kv_reinsert_after_evict(KV_AGE_BUCKETS[-1] + 1, 1)
        self.assertEqual(
...
def _install_tracker(cache, collector, page_size=1):
    tracker = KVGhostTracker(
        KVGhostList(capacity=64, ttl_seconds=3600.0), page_size, collector
    )
    return tracker
...
class TestRadixCacheGhostList(unittest.TestCase):
    ...
    def _build_cache(self):
        ...
        cache.kv_ghost = _install_tracker(cache, collector)
...
class TestUnifiedRadixCacheGhostList(unittest.TestCase):
    ...
    def _build_cache(self):
        ...
        cache.metrics_collector = collector
        cache.tree_core.kv_ghost = _install_tracker(cache, collector)
        return cache, allocator, collector
